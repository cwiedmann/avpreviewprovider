...
class voc extends handler
{
	...
	public function Analyze() {
		...
		if (substr($VOCheader, 0, 19) != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes(substr($VOCheader, 0, 19)).'"');
			return false;
		}
		...
		$thisfile_voc['header']['datablock_offset'] = lib::LittleEndian2Int(substr($VOCheader, 20, 2));
		$thisfile_voc['header']['minor_version']    = lib::LittleEndian2Int(substr($VOCheader, 22, 1));
		$thisfile_voc['header']['major_version']    = lib::LittleEndian2Int(substr($VOCheader, 23, 1));
		...
		do {
			...
			$BlockSize      = lib::LittleEndian2Int(substr($BlockData, 1, 3));
			$ThisBlock      = array();

			lib::safe_inc($thisfile_voc['blocktypes'][$BlockType], 1);
			switch ($BlockType) {
				case 0:  // Terminator
				...
				case 1:  // Sound data
					...
					$ThisBlock['sample_rate_id']   = lib::LittleEndian2Int(substr($BlockData, 4, 1));
					$ThisBlock['compression_type'] = lib::LittleEndian2Int(substr($BlockData, 5, 1));

					$ThisBlock['compression_name'] = $this->VOCcompressionTypeLookup($ThisBlock['compression_type']);
					if ($ThisBlock['compression_type'] <= 3) {
						$thisfile_voc['compressed_bits_per_sample'] = lib::CastAsInt(str_replace('-bit', '', $ThisBlock['compression_name']));
					}
					...
					if (empty($thisfile_audio['sample_rate'])) {
						...
						$thisfile_audio['sample_rate'] = lib::trunc((1000000 / (256 - $ThisBlock['sample_rate_id'])) / $thisfile_audio['channels']);
					}
					break;
				...
				case 8:  // Extended
					...
					$ThisBlock['time_constant'] =        lib::LittleEndian2Int(substr($BlockData, 4, 2));
					$ThisBlock['pack_method']   =        lib::LittleEndian2Int(substr($BlockData, 6, 1));
					$ThisBlock['stereo']        = (bool) lib::LittleEndian2Int(substr($BlockData, 7, 1));

					$thisfile_audio['channels']    = ($ThisBlock['stereo'] ? 2 : 1);
					$thisfile_audio['sample_rate'] = lib::trunc((256000000 / (65536 - $ThisBlock['time_constant'])) / $thisfile_audio['channels']);
					break;
				...
				case 9:  // data block that supersedes blocks 1 and 8. Used for stereo, 16 bit
					...
					$ThisBlock['sample_rate']      = lib::LittleEndian2Int(substr($BlockData,  4, 4));
					$ThisBlock['bits_per_sample']  = lib::LittleEndian2Int(substr($BlockData,  8, 1));
					$ThisBlock['channels']         = lib::LittleEndian2Int(substr($BlockData,  9, 1));
					$ThisBlock['wFormat']          = lib::LittleEndian2Int(substr($BlockData, 10, 2));
