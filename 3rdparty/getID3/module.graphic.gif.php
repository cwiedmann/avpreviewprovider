...
class gif extends handler
{
	...
	public function Analyze() {
		...
		if ($info['gif']['header']['raw']['identifier'] != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($info['gif']['header']['raw']['identifier']).'"');
		...
		$offset += 3;
		$info['gif']['header']['raw']['width']                 = lib::LittleEndian2Int(substr($GIFheader, $offset, 2));
		$offset += 2;
		$info['gif']['header']['raw']['height']                = lib::LittleEndian2Int(substr($GIFheader, $offset, 2));
		$offset += 2;
		$info['gif']['header']['raw']['flags']                 = lib::LittleEndian2Int(substr($GIFheader, $offset, 1));
		$offset += 1;
		$info['gif']['header']['raw']['bg_color_index']        = lib::LittleEndian2Int(substr($GIFheader, $offset, 1));
		$offset += 1;
		$info['gif']['header']['raw']['aspect_ratio']          = lib::LittleEndian2Int(substr($GIFheader, $offset, 1));
		$offset += 1;
		...
		if ($info['gif']['header']['flags']['global_color_table']) {
			...
			for ($i = 0; $i < $info['gif']['header']['global_color_size']; $i++) {
				$red   = lib::LittleEndian2Int(substr($GIFcolorTable, $offset++, 1));
				$green = lib::LittleEndian2Int(substr($GIFcolorTable, $offset++, 1));
				$blue  = lib::LittleEndian2Int(substr($GIFcolorTable, $offset++, 1));
...
/*
				case ',': // ',' - Image separator character
					...
					$ImageDescriptor['image_left']   = lib::LittleEndian2Int(substr($ImageDescriptorData, 1, 2));
					$ImageDescriptor['image_top']    = lib::LittleEndian2Int(substr($ImageDescriptorData, 3, 2));
					$ImageDescriptor['image_width']  = lib::LittleEndian2Int(substr($ImageDescriptorData, 5, 2));
					$ImageDescriptor['image_height'] = lib::LittleEndian2Int(substr($ImageDescriptorData, 7, 2));
					$ImageDescriptor['flags_raw']    = lib::LittleEndian2Int(substr($ImageDescriptorData, 9, 1));
					...
					$RasterData = array();
					$RasterData['code_size']        = lib::LittleEndian2Int($this->fread(1));
					$RasterData['block_byte_count'] = lib::LittleEndian2Int($this->fread(1));
...
*/
				...
				case '!':
					...
					$ExtensionBlock['function_code']  = lib::LittleEndian2Int(substr($ExtensionBlockData, 1, 1));
					$ExtensionBlock['byte_length']    = lib::LittleEndian2Int(substr($ExtensionBlockData, 2, 1));
					...
					if (substr($ExtensionBlock['data'], 0, 11) == 'NETSCAPE2.0') { // Netscape Application Block (NAB)
						...
						if (substr($ExtensionBlock['data'], 11, 2) == "\x03\x01") {
							...
							$info['gif']['animation']['loop_count'] = lib::LittleEndian2Int(substr($ExtensionBlock['data'], 13, 2));
						} else {
							$this->warning('Expecting 03 01 at offset '.($this->ftell() - 4).', found "'.lib::PrintHexBytes(substr($ExtensionBlock['data'], 11, 2)).'"');
						}
					}
