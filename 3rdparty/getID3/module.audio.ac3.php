...
class ac3 extends handler
{
	...
	public function Analyze() {
		...
		$this->AC3header['syncinfo']  =     lib::BigEndian2Int(substr($tempAC3header, 0, 2));
		$this->AC3header['bsi']       =     lib::BigEndian2Bin(substr($tempAC3header, 2));
		$thisfile_ac3_raw_bsi['bsid'] = (lib::LittleEndian2Int(substr($tempAC3header, 5, 1)) & 0xF8) >> 3; // AC3 and E-AC3 put the "bsid" version identifier in the same place, but unfortnately the 4 bytes between the syncword and the version identifier are interpreted differently, so grab it here so the following code structure can make sense
		unset($tempAC3header);
		...
		if ($thisfile_ac3_raw_bsi['bsid'] <= 8) {
			...
			$thisfile_ac3_raw_bsi['crc1']       = lib::Bin2Dec($this->readHeaderBSI(16));
			...
			if ($thisfile_ac3_raw_bsi['flags']['addbsi']) {
				...
				$this->AC3header['bsi'] .= lib::BigEndian2Bin($this->fread($thisfile_ac3_raw_bsi['addbsi_length']));
...
$thisfile_ac3['bitrate'] = round(($thisfile_ac3['bitrate'] * 1.05) / 16000) * 16000;
	...
	public static function heavyCompression($compre) {
		...
		$log_gain = ($log_gain + 1) * lib::RGADamplitude2dB(2);
