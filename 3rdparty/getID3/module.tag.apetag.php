...
class apetag extends handler
{
	...
	public function Analyze() {
		...
		for ($i = 0; $i < $thisfile_ape['footer']['raw']['tag_items']; $i++) {
			$value_size = lib::LittleEndian2Int(substr($APEtagData, $offset, 4));
			$offset += 4;
			$item_flags = lib::LittleEndian2Int(substr($APEtagData, $offset, 4));
			$offset += 4;
			...
			switch (strtolower($item_key)) {
				...
				case 'cover art (studio)':
					...
					do {
						...
						$imageinfo = array();
						$imagechunkcheck = lib::GetDataImageSize($thisfile_ape_items_current['data'], $imageinfo);
	...
	public function parseAPEheaderFooter($APEheaderFooterData) {
		...
		$headerfooterinfo_raw['version']      = lib::LittleEndian2Int(substr($APEheaderFooterData,  8, 4));
		$headerfooterinfo_raw['tagsize']      = lib::LittleEndian2Int(substr($APEheaderFooterData, 12, 4));
		$headerfooterinfo_raw['tag_items']    = lib::LittleEndian2Int(substr($APEheaderFooterData, 16, 4));
		$headerfooterinfo_raw['global_flags'] = lib::LittleEndian2Int(substr($APEheaderFooterData, 20, 4));
