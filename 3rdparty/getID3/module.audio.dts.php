    ...
    */
	...
	public function Analyze() {
        ...
        if (($encoding = array_search($sync, self::$syncwords)) !== false) {
			...
			return $this->error('Expecting "'.implode('| ', array_map('lib::PrintHexBytes', self::$syncwords)).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($sync).'"');
		...
		for ($word_offset = 0; $word_offset <= strlen($DTSheader); $word_offset += 2) {
			switch ($encoding) {
				case 0: // raw big-endian
					$fhBS .=        lib::BigEndian2Bin(       substr($DTSheader, $word_offset, 2) );
					break;
				case 1: // raw little-endian
					$fhBS .=        lib::BigEndian2Bin(strrev(substr($DTSheader, $word_offset, 2)));
					break;
				case 2: // 14-bit big-endian
					$fhBS .= substr(lib::BigEndian2Bin(       substr($DTSheader, $word_offset, 2) ), 2, 14);
					break;
				case 3: // 14-bit little-endian
					$fhBS .= substr(lib::BigEndian2Bin(strrev(substr($DTSheader, $word_offset, 2))), 2, 14);
					break;
			}
		}
