...
class gzip extends handler {
	...
	public function Analyze() {
		...
		for ($i = 0; $i < $num_members; $i++) {
			...
			$thisInfo['filemtime']      = lib::LittleEndian2Int($attr['mtime']);
			...
			if ($thisInfo['flags']['extra']) {
				...
				$xlen = lib::LittleEndian2Int($w_xlen);
				$fpointer += 2;
				...
				while (true) {
					...
					if (($si1 == 0x41) && ($si2 == 0x70)) {
						...
						$xsublen = lib::LittleEndian2Int($w_xsublen);
						$idx += 2;
			...
			if ($thisInfo['flags']['crc16']) {
				...
				$thisInfo['crc16'] = lib::LittleEndian2Int($w_crc);
				$fpointer += 2;
			}
			// bit 0 - FLG.FTEXT
			...
			$thisInfo['crc32']    = lib::LittleEndian2Int(substr($buff, strlen($buff) - 8, 4));
			$thisInfo['filesize'] = lib::LittleEndian2Int(substr($buff, strlen($buff) - 4));

			$info['gzip']['files'] = lib::array_merge_clobber($info['gzip']['files'], lib::CreateDeepArray($thisInfo['filename'], '/', $thisInfo['filesize']));
			...
			if ($this->option_gzip_parse_contents) {
				...
				if (function_exists('gzinflate')) {
					...
					switch ($determined_format['module']) {
						case 'tar':
							// view TAR-file info
							if (file_exists(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.$determined_format['include']) && include_once(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.$determined_format['include'])) {
								if (($temp_tar_filename = tempnam(AVPREVIEWPROVIDERGETID3_TEMP_DIR, 'getID3')) === false) {
