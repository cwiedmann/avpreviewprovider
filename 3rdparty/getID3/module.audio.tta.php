...
class tta extends handler
{
	...
	public function Analyze() {
		...
		if ($info['tta']['magic'] != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($info['tta']['magic']).'"');
		...
		switch ($ttaheader{3}) {
			...
			case "\x03": // TTA v1.x
				...
				$info['tta']['channels']            = lib::LittleEndian2Int(substr($ttaheader,  4,  2));
				$info['tta']['bits_per_sample']     = lib::LittleEndian2Int(substr($ttaheader,  6,  2));
				$info['tta']['sample_rate']         = lib::LittleEndian2Int(substr($ttaheader,  8,  4));
				$info['tta']['samples_per_channel'] = lib::LittleEndian2Int(substr($ttaheader, 12,  4));
			...
			case '2': // TTA v2.x
				...
				$info['tta']['compression_level']   = lib::LittleEndian2Int(substr($ttaheader,  4,  2));
				$info['tta']['audio_format']        = lib::LittleEndian2Int(substr($ttaheader,  6,  2));
				$info['tta']['channels']            = lib::LittleEndian2Int(substr($ttaheader,  8,  2));
				$info['tta']['bits_per_sample']     = lib::LittleEndian2Int(substr($ttaheader, 10,  2));
				$info['tta']['sample_rate']         = lib::LittleEndian2Int(substr($ttaheader, 12,  4));
				$info['tta']['data_length']         = lib::LittleEndian2Int(substr($ttaheader, 16,  4));
			...
			case '1': // TTA v3.x
				...
				$info['tta']['audio_format']        = lib::LittleEndian2Int(substr($ttaheader,  4,  2)); // riff::wFormatTagLookup()
				$info['tta']['channels']            = lib::LittleEndian2Int(substr($ttaheader,  6,  2));
				$info['tta']['bits_per_sample']     = lib::LittleEndian2Int(substr($ttaheader,  8,  2));
				$info['tta']['sample_rate']         = lib::LittleEndian2Int(substr($ttaheader, 10,  4));
				$info['tta']['data_length']         = lib::LittleEndian2Int(substr($ttaheader, 14,  4));
				$info['tta']['crc32_footer']        =                              substr($ttaheader, 18,  4);
				$info['tta']['seek_point']          = lib::LittleEndian2Int(substr($ttaheader, 22,  4));
