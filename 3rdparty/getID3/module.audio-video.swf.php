...
class swf extends handler
{
	...
	public function Analyze() {
		...
		switch ($info['swf']['header']['signature']) {
			...
			default:
				$this->error('Expecting "FWS" or "CWS" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($info['swf']['header']['signature']).'"');
				unset($info['swf']);
				unset($info['fileformat']);
				return false;
				break;
		}
		$info['swf']['header']['version'] = lib::LittleEndian2Int(substr($SWFfileData, 3, 1));
		$info['swf']['header']['length']  = lib::LittleEndian2Int(substr($SWFfileData, 4, 4));
		...
		$info['swf']['header']['frame_width']  = lib::Bin2Dec($X2);
		$info['swf']['header']['frame_height'] = lib::Bin2Dec($Y2);
		...
		$info['swf']['header']['frame_rate']  = lib::LittleEndian2Int(substr($SWFfileData,  9 + $FrameSizeDataLength, 1));
		$info['swf']['header']['frame_count'] = lib::LittleEndian2Int(substr($SWFfileData, 10 + $FrameSizeDataLength, 2));
...
//echo __LINE__.'='.number_format(microtime(true) - $start_time, 3).'<br>';
			...
			$TagIDTagLength = lib::LittleEndian2Int(substr($SWFfileData, $CurrentOffset, 2));
			$TagID     = ($TagIDTagLength & 0xFFFC) >> 6;
			$TagLength = ($TagIDTagLength & 0x003F);
			$CurrentOffset += 2;
			if ($TagLength == 0x3F) {
				$TagLength = lib::LittleEndian2Int(substr($SWFfileData, $CurrentOffset, 4));
				$CurrentOffset += 4;
			}
			...
			switch ($TagID) {
				...
				case 9: // Set background color
					...
					$info['swf']['bgcolor'] = strtoupper(str_pad(dechex(lib::BigEndian2Int($TagData['data'])), 6, '0', STR_PAD_LEFT));
					break;
