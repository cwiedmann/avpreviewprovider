...
class rkau extends handler
{
	...
	public function Analyze() {
		...
		if (substr($RKAUHeader, 0, 3) != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes(substr($RKAUHeader, 0, 3)).'"');
			return false;
		}
		...
		$info['rkau']['raw']['version']   = lib::LittleEndian2Int(substr($RKAUHeader, 3, 1));
		...
		$info['rkau']['source_bytes']     = lib::LittleEndian2Int(substr($RKAUHeader,  4, 4));
		$info['rkau']['sample_rate']      = lib::LittleEndian2Int(substr($RKAUHeader,  8, 4));
		$info['rkau']['channels']         = lib::LittleEndian2Int(substr($RKAUHeader, 12, 1));
		$info['rkau']['bits_per_sample']  = lib::LittleEndian2Int(substr($RKAUHeader, 13, 1));

		$info['rkau']['raw']['quality']   = lib::LittleEndian2Int(substr($RKAUHeader, 14, 1));
		$this->RKAUqualityLookup($info['rkau']);

		$info['rkau']['raw']['flags']            = lib::LittleEndian2Int(substr($RKAUHeader, 15, 1));
		...
		if ($info['rkau']['flags']['streaming']) {
			...
			$info['rkau']['compressed_bytes']  = lib::LittleEndian2Int(substr($RKAUHeader, 16, 4));
		} else {
