...
if (!defined('AVPREVIEWPROVIDERGETID3_INCLUDEPATH')) {
	throw new Exception('getid3.php MUST be included before calling writetags');
}
...
class writetags
{
	...
	public function WriteTags() {
		...
		if (empty($this->filename)) {
			$this->errors[] = 'filename is undefined in writetags';
			return false;
		} elseif (!file_exists($this->filename)) {
			$this->errors[] = 'filename set to non-existant file "'.$this->filename.'" in writetags';
			return false;
		}

		if (!is_array($this->tagformats)) {
			$this->errors[] = 'tagformats must be an array in writetags';
			return false;
		}
		...
		foreach ($WritingFilesToInclude as $tagformat) {
			switch ($tagformat) {
				case 'ape':
					$AVPREVIEWPROVIDERGETID3_ERRORARRAY = &$this->errors;
					lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'write.apetag.php', __FILE__, true);
					break;
				...
				case 'real':
					$AVPREVIEWPROVIDERGETID3_ERRORARRAY = &$this->errors;
					lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'write.'.$tagformat.'.php', __FILE__, true);
					break;
				...
				case 'id3v2':
					$AVPREVIEWPROVIDERGETID3_ERRORARRAY = &$this->errors;
					lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'write.id3v2.php', __FILE__, true);
					break;
...
throw new Exception('$this->overwrite_tags=false is known to be buggy in this version of getID3. Check http://github.com/JamesHeinrich/getID3 for a newer version.');
				...
				default:
					foreach ($valuearray as $key => $value) {
						if (is_string($value) || is_numeric($value)) {
							$ape_tag_data[$tag_key][$key] = lib::iconv_fallback($this->tag_encoding, 'UTF-8', $value);
						} else {
	...
	public function FormatDataForID3v1() {
		...
		$tag_data_id3v1['title']   =        lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['TITLE']      ) ? $this->tag_data['TITLE']       : array())));
		$tag_data_id3v1['artist']  =        lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['ARTIST']     ) ? $this->tag_data['ARTIST']      : array())));
		$tag_data_id3v1['album']   =        lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['ALBUM']      ) ? $this->tag_data['ALBUM']       : array())));
		$tag_data_id3v1['year']    =        lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['YEAR']       ) ? $this->tag_data['YEAR']        : array())));
		$tag_data_id3v1['comment'] =        lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['COMMENT']    ) ? $this->tag_data['COMMENT']     : array())));
		$tag_data_id3v1['track']   = intval(lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['TRACKNUMBER']) ? $this->tag_data['TRACKNUMBER'] : array()))));
						...
						if (isset($ID3v2_text_encoding_lookup[$id3v2_majorversion][$this->tag_encoding])) {
							...
							if ($id3v2_majorversion < 4) {
								...
								if (!$ID3v2_tag_data_converted) {
									...
									//$tag_data_id3v2[$ID3v2_framename][$key]['data']       = lib::iconv_fallback($this->tag_encoding, 'UTF-16', $value); // output is UTF-16LE+BOM or UTF-16BE+BOM depending on system architecture
									$tag_data_id3v2[$ID3v2_framename][$key]['data']       = "\xFF\xFE".lib::iconv_fallback($this->tag_encoding, 'UTF-16LE', $value); // force LittleEndian order version of UTF-16
									$ID3v2_tag_data_converted = true;
								}
								...
								$tag_data_id3v2[$ID3v2_framename][$key]['data']       = lib::iconv_fallback($this->tag_encoding, 'UTF-8', $value);
							}
						}
				...
				if (strstr($value, "\n")) {
					...
					foreach ($multilineexploded as $newcomment) {
						if (strlen(trim($newcomment)) > 0) {
							$tag_data_vorbiscomment[$tag_key][] = lib::iconv_fallback($this->tag_encoding, 'UTF-8', $newcomment);
						}
					}
				} elseif (is_string($value) || is_numeric($value)) {
					$tag_data_vorbiscomment[$tag_key][$key] = lib::iconv_fallback($this->tag_encoding, 'UTF-8', $value);
				} else {
	...
	public function FormatDataForReal() {
		$tag_data_real['title']     = lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['TITLE']    ) ? $this->tag_data['TITLE']     : array())));
		$tag_data_real['artist']    = lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['ARTIST']   ) ? $this->tag_data['ARTIST']    : array())));
		$tag_data_real['copyright'] = lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['COPYRIGHT']) ? $this->tag_data['COPYRIGHT'] : array())));
		$tag_data_real['comment']   = lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (isset($this->tag_data['COMMENT']  ) ? $this->tag_data['COMMENT']   : array())));
