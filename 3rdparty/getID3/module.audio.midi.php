...
define('AVPREVIEWPROVIDERGETID3_MIDI_MAGIC_MTHD', 'MThd'); // MIDI file header magic
define('AVPREVIEWPROVIDERGETID3_MIDI_MAGIC_MTRK', 'MTrk'); // MIDI track header magic
...
class midi extends handler
{
	...
	public function Analyze() {
		...
		if ($MIDIheaderID != AVPREVIEWPROVIDERGETID3_MIDI_MAGIC_MTHD) {
			$this->error('Expecting "'.lib::PrintHexBytes(AVPREVIEWPROVIDERGETID3_MIDI_MAGIC_MTHD).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($MIDIheaderID).'"');
			unset($info['fileformat']);
			return false;
		}
		$offset += 4;
		$thisfile_midi_raw['headersize']    = lib::BigEndian2Int(substr($MIDIdata, $offset, 4));
		$offset += 4;
		$thisfile_midi_raw['fileformat']    = lib::BigEndian2Int(substr($MIDIdata, $offset, 2));
		$offset += 2;
		$thisfile_midi_raw['tracks']        = lib::BigEndian2Int(substr($MIDIdata, $offset, 2));
		$offset += 2;
		$thisfile_midi_raw['ticksperqnote'] = lib::BigEndian2Int(substr($MIDIdata, $offset, 2));
		$offset += 2;
		...
		for ($i = 0; $i < $thisfile_midi_raw['tracks']; $i++) {
			...
			$offset += 4;
			if ($trackID == AVPREVIEWPROVIDERGETID3_MIDI_MAGIC_MTRK) {
				$tracksize = lib::BigEndian2Int(substr($MIDIdata, $offset, 4));
				$offset += 4;
				//$thisfile_midi['tracks'][$i]['size'] = $tracksize;
				$trackdataarray[$i] = substr($MIDIdata, $offset, $tracksize);
				$offset += $tracksize;
			} else {
				$this->error('Expecting "'.lib::PrintHexBytes(AVPREVIEWPROVIDERGETID3_MIDI_MAGIC_MTRK).'" at '.($offset - 4).', found "'.lib::PrintHexBytes($trackID).'" instead');
				return false;
			}
		}
					...
					if ($MIDIevents[$tracknumber][$eventid]['eventid'] == 0x08) { // Note off (key is released)
						...
						switch ($METAeventCommand) {
							case 0x00: // Set track sequence number
								$track_sequence_number = lib::BigEndian2Int(substr($METAeventData, 0, $METAeventLength));
							...
							case 0x51: // Tempo: microseconds / quarter note
								$CurrentMicroSecondsPerBeat = lib::BigEndian2Int(substr($METAeventData, 0, $METAeventLength));
							...
							case 0x58: // Time signature
								$timesig_numerator   = lib::BigEndian2Int($METAeventData{0});
								$timesig_denominator = pow(2, lib::BigEndian2Int($METAeventData{1})); // $02 -> x/4, $03 -> x/8, etc
								$timesig_32inqnote   = lib::BigEndian2Int($METAeventData{2});         // number of 32nd notes to the quarter note
							...
							case 0x59: // Keysignature
								$keysig_sharpsflats = lib::BigEndian2Int($METAeventData{0});
								if ($keysig_sharpsflats & 0x80) {
									// (-7 -> 7 flats, 0 ->key of C, 7 -> 7 sharps)
									$keysig_sharpsflats -= 256;
								}

								$keysig_majorminor  = lib::BigEndian2Int($METAeventData{1}); // 0 -> major, 1 -> minor
	...
	public function GeneralMIDIinstrumentLookup($instrumentid) {
		...
		*/
		...
		return lib::EmbeddedLookup($instrumentid, $begin, __LINE__, __FILE__, 'GeneralMIDIinstrument');
	}
	...
	public function GeneralMIDIpercussionLookup($instrumentid) {
		...
		*/
		...
		return lib::EmbeddedLookup($instrumentid, $begin, __LINE__, __FILE__, 'GeneralMIDIpercussion');
	}
