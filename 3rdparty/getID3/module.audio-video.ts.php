...
class ts extends handler
{
	...
	public function Analyze() {
		...
		if (substr($TSheader, 0, 1) != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at '.$info['avdataoffset'].', found '.lib::PrintHexBytes(substr($TSheader, 0, 1)).' instead.');
			return false;
		}
		$info['fileformat'] = 'ts';
		...
		$offset = 0;
		$info['ts']['packet']['sync'] = lib::BigEndian2Int(substr($TSheader, $offset, 1)); $offset += 1;
		$pid_flags_raw                = lib::BigEndian2Int(substr($TSheader, $offset, 2)); $offset += 2;
		$SAC_raw                      = lib::BigEndian2Int(substr($TSheader, $offset, 1)); $offset += 1;
		...
		if ($info['ts']['packet']['flags']['adaption_field_exists']) {
			$AdaptionField_raw        = lib::BigEndian2Int(substr($TSheader, $offset, 2)); $offset += 2;
			...
			if ($info['ts']['packet']['adaption']['flags']['pcr']) {
				$info['ts']['packet']['adaption']['raw']['pcr'] = lib::BigEndian2Int(substr($TSheader, $offset, 6)); $offset += 6;
			}
			if ($info['ts']['packet']['adaption']['flags']['opcr']) {
				$info['ts']['packet']['adaption']['raw']['opcr'] = lib::BigEndian2Int(substr($TSheader, $offset, 6)); $offset += 6;
			}
		}
