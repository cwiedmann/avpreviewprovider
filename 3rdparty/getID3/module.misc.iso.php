...
class iso extends handler
{
	...
	public function ParsePrimaryVolumeDescriptor(&$ISOheader) {
		...
		$thisfile_iso_primaryVD_raw['volume_descriptor_type']         = lib::LittleEndian2Int(substr($ISOheader,    0, 1));
		...
		$thisfile_iso_primaryVD_raw['volume_descriptor_version']     = lib::LittleEndian2Int(substr($ISOheader,    6, 1));
		...
		$thisfile_iso_primaryVD_raw['volume_space_size']             = lib::LittleEndian2Int(substr($ISOheader,   80, 4));
		//$thisfile_iso_primaryVD_raw['unused_3']                      =                              substr($ISOheader,   88, 32);
		$thisfile_iso_primaryVD_raw['volume_set_size']               = lib::LittleEndian2Int(substr($ISOheader,  120, 2));
		$thisfile_iso_primaryVD_raw['volume_sequence_number']        = lib::LittleEndian2Int(substr($ISOheader,  124, 2));
		$thisfile_iso_primaryVD_raw['logical_block_size']            = lib::LittleEndian2Int(substr($ISOheader,  128, 2));
		$thisfile_iso_primaryVD_raw['path_table_size']               = lib::LittleEndian2Int(substr($ISOheader,  132, 4));
		$thisfile_iso_primaryVD_raw['path_table_l_location']         = lib::LittleEndian2Int(substr($ISOheader,  140, 2));
		$thisfile_iso_primaryVD_raw['path_table_l_opt_location']     = lib::LittleEndian2Int(substr($ISOheader,  144, 2));
		$thisfile_iso_primaryVD_raw['path_table_m_location']         = lib::LittleEndian2Int(substr($ISOheader,  148, 2));
		$thisfile_iso_primaryVD_raw['path_table_m_opt_location']     = lib::LittleEndian2Int(substr($ISOheader,  152, 2));
		...
		$thisfile_iso_primaryVD_raw['file_structure_version']        = lib::LittleEndian2Int(substr($ISOheader,  881, 1));
		//$thisfile_iso_primaryVD_raw['unused_4']                      = lib::LittleEndian2Int(substr($ISOheader,  882, 1));
	...
	public function ParseSupplementaryVolumeDescriptor(&$ISOheader) {
		...
		$thisfile_iso_supplementaryVD_raw['volume_descriptor_type'] = lib::LittleEndian2Int(substr($ISOheader,    0, 1));
		...
		$thisfile_iso_supplementaryVD_raw['volume_descriptor_version'] = lib::LittleEndian2Int(substr($ISOheader,    6, 1));
		...
		$thisfile_iso_supplementaryVD_raw['volume_space_size']         = lib::LittleEndian2Int(substr($ISOheader,   80, 4));
		...
		$thisfile_iso_supplementaryVD_raw['volume_set_size']                = lib::LittleEndian2Int(substr($ISOheader,  120, 2));
		$thisfile_iso_supplementaryVD_raw['volume_sequence_number']         = lib::LittleEndian2Int(substr($ISOheader,  124, 2));
		$thisfile_iso_supplementaryVD_raw['logical_block_size']             = lib::LittleEndian2Int(substr($ISOheader,  128, 2));
		$thisfile_iso_supplementaryVD_raw['path_table_size']                = lib::LittleEndian2Int(substr($ISOheader,  132, 4));
		$thisfile_iso_supplementaryVD_raw['path_table_l_location']          = lib::LittleEndian2Int(substr($ISOheader,  140, 2));
		$thisfile_iso_supplementaryVD_raw['path_table_l_opt_location']      = lib::LittleEndian2Int(substr($ISOheader,  144, 2));
		$thisfile_iso_supplementaryVD_raw['path_table_m_location']          = lib::LittleEndian2Int(substr($ISOheader,  148, 2));
		$thisfile_iso_supplementaryVD_raw['path_table_m_opt_location']      = lib::LittleEndian2Int(substr($ISOheader,  152, 2));
		...
		$thisfile_iso_supplementaryVD_raw['file_structure_version']         = lib::LittleEndian2Int(substr($ISOheader,  881, 1));
		//$thisfile_iso_supplementaryVD_raw['unused_4']                       = lib::LittleEndian2Int(substr($ISOheader,  882, 1));
	...
	public function ParsePathTable() {
		...
		while ($offset < $PathTableSize) {
			...
			$thisfile_iso_pathtable_directories_current['length']           = lib::LittleEndian2Int(substr($info['iso']['path_table']['raw'], $offset, 1));
			$offset += 1;
			$thisfile_iso_pathtable_directories_current['extended_length']  = lib::LittleEndian2Int(substr($info['iso']['path_table']['raw'], $offset, 1));
			$offset += 1;
			$thisfile_iso_pathtable_directories_current['location_logical'] = lib::LittleEndian2Int(substr($info['iso']['path_table']['raw'], $offset, 4));
			$offset += 4;
			$thisfile_iso_pathtable_directories_current['parent_directory'] = lib::LittleEndian2Int(substr($info['iso']['path_table']['raw'], $offset, 2));
			$offset += 2;
			...
			$thisfile_iso_pathtable_directories_current['name_ascii']       = lib::iconv_fallback($TextEncoding, $info['encoding'], $thisfile_iso_pathtable_directories_current['name']);
	...
	public function ParseDirectoryRecord($directorydata) {
		...
		while (ord($DirectoryRecordData{0}) > 33) {
			...
			$ThisDirectoryRecord['raw']['length']                    = lib::LittleEndian2Int(substr($DirectoryRecordData,  0, 1));
			$ThisDirectoryRecord['raw']['extended_attribute_length'] = lib::LittleEndian2Int(substr($DirectoryRecordData,  1, 1));
			$ThisDirectoryRecord['raw']['offset_logical']            = lib::LittleEndian2Int(substr($DirectoryRecordData,  2, 4));
			$ThisDirectoryRecord['raw']['filesize']                  = lib::LittleEndian2Int(substr($DirectoryRecordData, 10, 4));
			$ThisDirectoryRecord['raw']['recording_date_time']       =                  substr($DirectoryRecordData, 18, 7);
			$ThisDirectoryRecord['raw']['file_flags']                = lib::LittleEndian2Int(substr($DirectoryRecordData, 25, 1));
			$ThisDirectoryRecord['raw']['file_unit_size']            = lib::LittleEndian2Int(substr($DirectoryRecordData, 26, 1));
			$ThisDirectoryRecord['raw']['interleave_gap_size']       = lib::LittleEndian2Int(substr($DirectoryRecordData, 27, 1));
			$ThisDirectoryRecord['raw']['volume_sequence_number']    = lib::LittleEndian2Int(substr($DirectoryRecordData, 28, 2));
			$ThisDirectoryRecord['raw']['file_identifier_length']    = lib::LittleEndian2Int(substr($DirectoryRecordData, 32, 1));
			$ThisDirectoryRecord['raw']['file_identifier']           =                  substr($DirectoryRecordData, 33, $ThisDirectoryRecord['raw']['file_identifier_length']);

			$ThisDirectoryRecord['file_identifier_ascii']            = lib::iconv_fallback($TextEncoding, $info['encoding'], $ThisDirectoryRecord['raw']['file_identifier']);
			...
			if ($ThisDirectoryRecord['file_flags']['directory']) {
				...
				$info['iso']['files'] = lib::array_merge_clobber($info['iso']['files'], lib::CreateDeepArray($ThisDirectoryRecord['filename'], '/', $ThisDirectoryRecord['filesize']));
			}
