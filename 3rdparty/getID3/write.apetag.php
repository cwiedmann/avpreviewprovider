...
lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.tag.apetag.php', __FILE__, true);
...
class apetag
{
	...
	public function GenerateAPEtag() {
		...
		foreach ($this->tag_data as $key => $arrayofvalues) {
			...
			$tagitem  = lib::LittleEndian2String(strlen($valuestring), 4);
	...
	public function GenerateAPEtagHeaderFooter(&$items, $isheader=false) {
		...
		$APEheader  = 'APETAGEX';
		$APEheader .= lib::LittleEndian2String(2000, 4);
		$APEheader .= lib::LittleEndian2String(32 + $tagdatalength, 4);
		$APEheader .= lib::LittleEndian2String(count($items), 4);
