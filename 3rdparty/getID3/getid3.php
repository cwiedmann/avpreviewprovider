...
if (!defined('AVPREVIEWPROVIDERGETID3_OS_ISWINDOWS')) {
	define('AVPREVIEWPROVIDERGETID3_OS_ISWINDOWS', (stripos(PHP_OS, 'WIN') === 0));
}
// Get base path of getID3() - ONCE
if (!defined('AVPREVIEWPROVIDERGETID3_INCLUDEPATH')) {
	define('AVPREVIEWPROVIDERGETID3_INCLUDEPATH', dirname(__FILE__).DIRECTORY_SEPARATOR);
}
...
if (!defined('AVPREVIEWPROVIDERGETID3_TEMP_DIR')) {
	define('AVPREVIEWPROVIDERGETID3_TEMP_DIR', $temp_dir);
}
...
class getID3
{
	...
	public $tempdir = AVPREVIEWPROVIDERGETID3_TEMP_DIR;
	...
	public function __construct() {
		...
		if (!include_once(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'getid3.lib.php')) {
		...
		if (AVPREVIEWPROVIDERGETID3_OS_ISWINDOWS && !defined('AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR')) {

			$helperappsdir = AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'..'.DIRECTORY_SEPARATOR.'helperapps'; // must not have any space in this path

			if (!is_dir($helperappsdir)) {
				$this->startup_warning .= '"'.$helperappsdir.'" cannot be defined as AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR because it does not exist'."\n";
				...
				foreach ($DirPieces as $key => $value) {
					if (strpos($value, ' ') !== false) {
						if (!empty($path_so_far)) {
							...
							$this->startup_warning .= 'AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR must not have any spaces in it - use 8dot3 naming convention if neccesary. You can run "dir /x" from the commandline to see the correct 8.3-style names.'."\n";
						}
					}
					$path_so_far[] = $value;
				}
			...
			define('AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR', $helperappsdir.DIRECTORY_SEPARATOR);
		}
	...
	public function openfile($filename, $filesize=null) {
		try {
			...
			$this->info = array();
			$this->info['AVPREVIEWPROVIDERGETID3_VERSION']   = $this->version();
			...
			$this->info['filename']     = lib::mb_basename($filename);
			...
			if ($this->option_max_2gb_check) {
				...
				if (($fseek < 0) || (($this->info['filesize'] != 0) && (ftell($this->fp) == 0)) ||
					...
					(ftell($this->fp) < 0)) {
						$real_filesize = lib::getFileSizeSyscall($this->info['filenamepath']);
						...
						} elseif (lib::intValueSupported($real_filesize)) {
							unset($this->info['filesize']);
							fclose($this->fp);
		...
		try {
			...
			foreach (array('id3v2'=>'id3v2', 'id3v1'=>'id3v1', 'apetag'=>'ape', 'lyrics3'=>'lyrics3') as $tag_name => $tag_key) {
				...
				if ($this->$option_tag) {
					...
					try {
						$tag_class = '\OCA\AvPreviewProvider\getID3\\'.$tag_name;
			...
			if (!$this->option_tag_id3v2) {
				...
				if ((substr($header, 0, 3) == 'ID3') && (strlen($header) == 10)) {
					...
					$this->info['avdataoffset']          += lib::BigEndian2Int(substr($header, 6, 4), 1) + 10; // length of ID3v2 tag in 10-byte header doesn't include 10-byte header length
				}
			}
			...
			if (!file_exists(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.$determined_format['include'])) {
				fclose($this->fp);
			...
			if (!empty($determined_format['iconv_req']) && !function_exists('mb_convert_encoding') && !function_exists('iconv') && !in_array($this->encoding, array('ISO-8859-1', 'UTF-8', 'UTF-16LE', 'UTF-16BE', 'UTF-16'))) {
				...
				if (AVPREVIEWPROVIDERGETID3_OS_ISWINDOWS) {
			...
			// include module
			include_once(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.$determined_format['include']);

			// instantiate module class
			$class_name = '\OCA\AvPreviewProvider\getID3\\'.$determined_format['module'];
...
//						),
	...
	public function CharConvert(&$array, $encoding) {
		...
		foreach ($array as $key => $value) {
			...
			elseif (is_string($value)) {
				$array[$key] = trim(lib::iconv_fallback($encoding, $this->encoding, $value));
			}
		}
	}
			...
			if (!empty($this->info[$comment_name]['comments'])) {
				...
				if ($this->option_tags_html) {
					foreach ($this->info['tags'][$tag_name] as $tag_key => $valuearray) {
						$this->info['tags_html'][$tag_name][$tag_key] = lib::recursiveMultiByteCharString2HTML($valuearray, $this->info[$comment_name]['encoding']);
					}
				}
	...
	public function getHashdata($algorithm) {
		...
		if (!empty($this->info['fileformat']) && !empty($this->info['dataformat']) && ($this->info['fileformat'] == 'ogg') && ($this->info['audio']['dataformat'] == 'vorbis')) {
			...
			if (preg_match('#(1|ON)#i', ini_get('safe_mode'))) {
				...
				// Create empty file
				$empty = tempnam(AVPREVIEWPROVIDERGETID3_TEMP_DIR, 'getID3');
				touch($empty);

				// Use vorbiscomment to make temp file without comments
				$temp = tempnam(AVPREVIEWPROVIDERGETID3_TEMP_DIR, 'getID3');
				$file = $this->info['filenamepath'];

				if (AVPREVIEWPROVIDERGETID3_OS_ISWINDOWS) {

					if (file_exists(AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR.'vorbiscomment.exe')) {

						$commandline = '"'.AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR.'vorbiscomment.exe" -w -c "'.$empty.'" "'.$file.'" "'.$temp.'"';
						$VorbisCommentError = `$commandline`;

					} else {

						$VorbisCommentError = 'vorbiscomment.exe not found in '.AVPREVIEWPROVIDERGETID3_HELPERAPPSDIR;
			...
			if (!empty($this->info['avdataoffset']) || (isset($this->info['avdataend']) && ($this->info['avdataend'] < $this->info['filesize']))) {
				...
				$this->info[$algorithm.'_data'] = lib::hash_data($this->info['filenamepath'], $this->info['avdataoffset'], $this->info['avdataend'], $algorithm);
	...
	public function ChannelsBitratePlaytimeCalculations() {
		...
		if (!empty($this->info['playtime_seconds']) && empty($this->info['playtime_string'])) {
			$this->info['playtime_string'] = lib::PlaytimeString($this->info['playtime_seconds']);
		}
	}
	...
	public function CalculateReplayGain() {
		if (isset($this->info['replay_gain'])) {
			...
			if (isset($this->info['replay_gain']['track']['peak'])) {
				$this->info['replay_gain']['track']['max_noclip_gain'] = 0 - lib::RGADamplitude2dB($this->info['replay_gain']['track']['peak']);
			}
			if (isset($this->info['replay_gain']['album']['peak'])) {
				$this->info['replay_gain']['album']['max_noclip_gain'] = 0 - lib::RGADamplitude2dB($this->info['replay_gain']['album']['peak']);
			}
		}
		return true;
	}
	...
	public function include_module($name) {
		...
		if (!file_exists(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.'.$name.'.php')) {
			throw new exception('Required module.'.$name.'.php is missing.');
		}
		include_once(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.'.$name.'.php');
		return true;
	}
...
abstract class handler {
	...
	*/
	...
	public function __construct(getID3 $getid3, $call_module=null) {
		...
		if ($call_module) {
			$this->dependency_to = str_replace('\OCA\AvPreviewProvider\getID3\\', '', $call_module);
		}
	}
	...
	public function saveAttachment($name, $offset, $length, $image_mime=null) {
		try {
			...
			// assume directory path is given
				...
				$dest = $dir.DIRECTORY_SEPARATOR.$name.($image_mime ? '.'.lib::ImageExtFromMime($image_mime) : '');
