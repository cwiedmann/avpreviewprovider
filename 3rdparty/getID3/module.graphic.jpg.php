...
class jpg extends handler
{
	...
	public function Analyze() {
		...
		$imageinfo = array();
		//list($width, $height, $type) = lib::GetDataImageSize($this->fread($info['filesize']), $imageinfo);
...
//return false;
						...
						$this->warning('EXIF parsing only available when '.(AVPREVIEWPROVIDERGETID3_OS_ISWINDOWS ? 'php_exif.dll enabled' : 'compiled with --enable-exif'));
					}
				}
				$returnOK = true;
				break;
		...
		if (isset($info['jpg']['exif']['GPS'])) {
			...
			if (isset($info['jpg']['exif']['GPS']['GPSDateStamp'])) {
				...
				if (isset($info['jpg']['exif']['GPS']['GPSTimeStamp']) && is_array($info['jpg']['exif']['GPS']['GPSTimeStamp'])) {
					foreach ($info['jpg']['exif']['GPS']['GPSTimeStamp'] as $key => $value) {
						$computed_time[$key] = lib::DecimalizeFraction($value);
					}
				}
			...
			if (isset($info['jpg']['exif']['GPS']['GPSLatitude']) && is_array($info['jpg']['exif']['GPS']['GPSLatitude'])) {
				...
				foreach ($info['jpg']['exif']['GPS']['GPSLatitude'] as $key => $value) {
					$computed_latitude[$key] = lib::DecimalizeFraction($value);
				}
			...
			if (isset($info['jpg']['exif']['GPS']['GPSLongitude']) && is_array($info['jpg']['exif']['GPS']['GPSLongitude'])) {
				...
				foreach ($info['jpg']['exif']['GPS']['GPSLongitude'] as $key => $value) {
					$computed_longitude[$key] = lib::DecimalizeFraction($value);
				}
			...
			if (isset($info['jpg']['exif']['GPS']['GPSAltitude'])) {
				...
				$info['jpg']['exif']['GPS']['computed']['altitude'] = $direction_multiplier * lib::DecimalizeFraction($info['jpg']['exif']['GPS']['GPSAltitude']);
			}

		}


		lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.tag.xmp.php', __FILE__, true);
	...
	public function CastAsAppropriate($value) {
		if (is_array($value)) {
			...
			return lib::DecimalizeFraction($value);
		} elseif (preg_match('#^[0-9]+$#', $value)) {
			return lib::CastAsInt($value);
		} elseif (preg_match('#^[0-9\.]+$#', $value)) {
			return (float) $value;
		}
		return $value;
