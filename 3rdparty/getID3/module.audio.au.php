...
class au extends handler
{
	...
	public function Analyze() {
		...
		if (substr($AUheader, 0, 4) != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" (".snd") at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes(substr($AUheader, 0, 4)).'"');
			return false;
		}
		...
		$thisfile_au['header_length']   = lib::BigEndian2Int(substr($AUheader,  4, 4));
		$AUheader .= $this->fread($thisfile_au['header_length'] - 8);
		$info['avdataoffset'] += $thisfile_au['header_length'];

		$thisfile_au['data_size']             = lib::BigEndian2Int(substr($AUheader,  8, 4));
		$thisfile_au['data_format_id']        = lib::BigEndian2Int(substr($AUheader, 12, 4));
		$thisfile_au['sample_rate']           = lib::BigEndian2Int(substr($AUheader, 16, 4));
		$thisfile_au['channels']              = lib::BigEndian2Int(substr($AUheader, 20, 4));
