...
class avr extends handler
{
	...
	public function Analyze() {
		...
		if ($info['avr']['raw']['magic'] != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($info['avr']['raw']['magic']).'"');
		...
		$info['avr']['raw']['mono']        = lib::BigEndian2Int(substr($AVRheader, 12,  2));
		$info['avr']['bits_per_sample']    = lib::BigEndian2Int(substr($AVRheader, 14,  2));
		$info['avr']['raw']['signed']      = lib::BigEndian2Int(substr($AVRheader, 16,  2));
		$info['avr']['raw']['loop']        = lib::BigEndian2Int(substr($AVRheader, 18,  2));
		$info['avr']['raw']['midi']        = lib::BigEndian2Int(substr($AVRheader, 20,  2));
		$info['avr']['raw']['replay_freq'] = lib::BigEndian2Int(substr($AVRheader, 22,  1));
		$info['avr']['sample_rate']        = lib::BigEndian2Int(substr($AVRheader, 23,  3));
		$info['avr']['sample_length']      = lib::BigEndian2Int(substr($AVRheader, 26,  4));
		$info['avr']['loop_start']         = lib::BigEndian2Int(substr($AVRheader, 30,  4));
		$info['avr']['loop_end']           = lib::BigEndian2Int(substr($AVRheader, 34,  4));
		$info['avr']['midi_split']         = lib::BigEndian2Int(substr($AVRheader, 38,  2));
		$info['avr']['sample_compression'] = lib::BigEndian2Int(substr($AVRheader, 40,  2));
		$info['avr']['reserved']           = lib::BigEndian2Int(substr($AVRheader, 42,  2));
