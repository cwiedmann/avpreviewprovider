...
lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.audio.mp3.php', __FILE__, true);
lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.tag.id3v2.php', __FILE__, true); // needed for ISO 639-2 language code lookup
...
class quicktime extends handler
{
	...
	public function Analyze() {
		...
		while ($offset < $info['avdataend']) {
			if (!lib::intValueSupported($offset)) {
			...
			$atomsize = lib::BigEndian2Int(substr($AtomHeader, 0, 4));
			$atomname = substr($AtomHeader, 4, 4);

			// 64-bit MOV patch by jlegateØktnc*com
			if ($atomsize == 1) {
				$atomsize = lib::BigEndian2Int($this->fread(8));
			}
	...
	public function QuicktimeParseAtom($atomname, $atomsize, $atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
		...
		switch ($atomname) {
			...
			case "\x00\x00\x00\x05":
				$atomname = lib::BigEndian2Int($atomname);
				$atom_structure['name'] = $atomname;
				$atom_structure['subatoms'] = $this->QuicktimeParseContainerAtom($atom_data, $baseoffset + 8, $atomHierarchy, $ParseAllPossibleAtoms);
				break;
			...
			case 'tvsn': // TV SeasoN
				if ($atom_parent == 'udta') {
					...
					$atom_structure['data_length'] = lib::BigEndian2Int(substr($atom_data, 0, 2));
					$atom_structure['language_id'] = lib::BigEndian2Int(substr($atom_data, 2, 2));
					...
					if (substr($atom_data, 2, 2) == "\x10\xB5") {
						...
						while ($atomoffset < strlen($atom_data)) {
							$boxsmallsize = lib::BigEndian2Int(substr($atom_data, $atomoffset,     2));
							...
							switch ($boxsmalltype) {
								case "\x10\xB5":
									...
									break;
								default:
									$this->warning('Unknown QuickTime smallbox type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxsmalltype).'" ('.trim(lib::PrintHexBytes($boxsmalltype)).') at offset '.$baseoffset);
						...
						while ($atomoffset < strlen($atom_data)) {
							$boxsize = lib::BigEndian2Int(substr($atom_data, $atomoffset, 4));
							...
							switch ($boxtype) {
								...
								case 'data':
									$atom_structure['version']   = lib::BigEndian2Int(substr($boxdata,  0, 1));
									$atom_structure['flags_raw'] = lib::BigEndian2Int(substr($boxdata,  1, 3));
										...
										case 21: // tmpo/cpil flag
											switch ($atomname) {
												...
												case 'pgap':
													...
													$atom_structure['data'] = lib::BigEndian2Int(substr($boxdata, 8, 1));
													break;

												case 'tmpo':
													// 16-bit integer
													$atom_structure['data'] = lib::BigEndian2Int(substr($boxdata, 8, 2));
													break;

												case 'disk':
												case 'trkn':
													// binary
													$num       = lib::BigEndian2Int(substr($boxdata, 10, 2));
													$num_total = lib::BigEndian2Int(substr($boxdata, 12, 2));
												...
												case 'gnre':
													// enum
													$GenreID = lib::BigEndian2Int(substr($boxdata, 8, 4));
													$atom_structure['data']    = id3v1::LookupGenreName($GenreID - 1);
													break;

												case 'rtng':
													// 8-bit integer
													$atom_structure[$atomname] = lib::BigEndian2Int(substr($boxdata, 8, 1));
												...
												case 'sfID':
													// 32-bit integer
													$atom_structure[$atomname] = lib::BigEndian2Int(substr($boxdata, 8, 4));
												...
												case 'plID':
													// 64-bit integer
													$atom_structure['data'] = lib::BigEndian2Int(substr($boxdata, 8, 8));
													break;
												...
												default:
													// 32-bit integer
													$atom_structure['data'] = lib::BigEndian2Int(substr($boxdata, 8, 4));
											}
											break;
								...
								default:
									$this->warning('Unknown QuickTime box type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxtype).'" ('.trim(lib::PrintHexBytes($boxtype)).') at offset '.$baseoffset);
									$atom_structure['data'] = $atom_data;
			...
			case 'play': // auto-PLAY atom
				$atom_structure['autoplay'] = (bool) lib::BigEndian2Int(substr($atom_data,  0, 1));
			...
			case 'WLOC': // Window LOCation atom
				$atom_structure['location_x']  = lib::BigEndian2Int(substr($atom_data,  0, 2));
				$atom_structure['location_y']  = lib::BigEndian2Int(substr($atom_data,  2, 2));
				break;
			...
			case 'AllF': // play ALL Frames atom
				$atom_structure['data'] = lib::BigEndian2Int($atom_data);
				break;
			...
			case 'cmvd': // Compressed MooV Data atom
				...
				$atom_structure['unCompressedSize'] = lib::BigEndian2Int(substr($atom_data, 0, 4));
			...
			case 'rdrf': // Reference movie Data ReFerence atom
				$atom_structure['version']                = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']              = lib::BigEndian2Int(substr($atom_data,  1, 3));
				$atom_structure['flags']['internal_data'] = (bool) ($atom_structure['flags_raw'] & 0x000001);

				$atom_structure['reference_type_name']    =                           substr($atom_data,  4, 4);
				$atom_structure['reference_length']       = lib::BigEndian2Int(substr($atom_data,  8, 4));
			...
			case 'rmqu': // Reference Movie QUality atom
				$atom_structure['movie_quality'] = lib::BigEndian2Int($atom_data);
				break;


			case 'rmcs': // Reference Movie Cpu Speed atom
				$atom_structure['version']          = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']        = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['cpu_speed_rating'] = lib::BigEndian2Int(substr($atom_data,  4, 2));
				break;
			...
			case 'rmvc': // Reference Movie Version Check atom
				...
				$atom_structure['gestalt_value_mask'] = lib::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['gestalt_value']      = lib::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['gestalt_check_type'] = lib::BigEndian2Int(substr($atom_data, 14, 2));
				break;
			...
			case 'rmcd': // Reference Movie Component check atom
				...
				$atom_structure['component_flags_raw']    = lib::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['component_flags_mask']   = lib::BigEndian2Int(substr($atom_data, 20, 4));
				$atom_structure['component_min_version']  = lib::BigEndian2Int(substr($atom_data, 24, 4));
				break;
			...
			case 'rmdr': // Reference Movie Data Rate atom
				$atom_structure['version']       = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']     = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['data_rate']     = lib::BigEndian2Int(substr($atom_data,  4, 4));
			...
			case 'rmla': // Reference Movie Language Atom
				$atom_structure['version']     = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']   = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['language_id'] = lib::BigEndian2Int(substr($atom_data,  4, 2));
			...
			case 'rmla': // Reference Movie Language Atom
				$atom_structure['version']   = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw'] = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['track_id']  = lib::BigEndian2Int(substr($atom_data,  4, 2));
				break;


			case 'ptv ': // Print To Video - defines a movie's full screen mode
				// http://developer.apple.com/documentation/QuickTime/APIREF/SOURCESIV/at_ptv-_pg.htm
				$atom_structure['display_size_raw']  = lib::BigEndian2Int(substr($atom_data, 0, 2));
				$atom_structure['reserved_1']        = lib::BigEndian2Int(substr($atom_data, 2, 2)); // hardcoded: 0x0000
				$atom_structure['reserved_2']        = lib::BigEndian2Int(substr($atom_data, 4, 2)); // hardcoded: 0x0000
				$atom_structure['slide_show_flag']   = lib::BigEndian2Int(substr($atom_data, 6, 1));
				$atom_structure['play_on_open_flag'] = lib::BigEndian2Int(substr($atom_data, 7, 1));
			...
			case 'stsd': // Sample Table Sample Description atom
				$atom_structure['version']        = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']      = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['number_entries'] = lib::BigEndian2Int(substr($atom_data,  4, 4));
				...
				if ($atom_structure['number_entries'] > 0x000FFFFF) {
					if ($atom_structure['number_entries'] > 0x00FFFFFF) {
						$this->warning('"stsd" atom contains improbably large number_entries (0x'.lib::PrintHexBytes(substr($atom_data, 4, 4), true, false).' = '.$atom_structure['number_entries'].'), probably in error. Ignoring upper byte and interpreting this as 0x'.lib::PrintHexBytes(substr($atom_data, 5, 3), true, false).' = '.($atom_structure['number_entries'] & 0x00FFFFFF));
						$atom_structure['number_entries'] = ($atom_structure['number_entries'] & 0x00FFFFFF);
					} else {
						$this->warning('"stsd" atom contains improbably large number_entries (0x'.lib::PrintHexBytes(substr($atom_data, 4, 4), true, false).' = '.$atom_structure['number_entries'].'), probably in error. Please report this to info@getid3.org referencing bug report #111');
					}
				}

				$stsdEntriesDataOffset = 8;
				for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
					$atom_structure['sample_description_table'][$i]['size']             = lib::BigEndian2Int(substr($atom_data, $stsdEntriesDataOffset, 4));
					$stsdEntriesDataOffset += 4;
					$atom_structure['sample_description_table'][$i]['data_format']      =                           substr($atom_data, $stsdEntriesDataOffset, 4);
					$stsdEntriesDataOffset += 4;
					$atom_structure['sample_description_table'][$i]['reserved']         = lib::BigEndian2Int(substr($atom_data, $stsdEntriesDataOffset, 6));
					$stsdEntriesDataOffset += 6;
					$atom_structure['sample_description_table'][$i]['reference_index']  = lib::BigEndian2Int(substr($atom_data, $stsdEntriesDataOffset, 2));
					$stsdEntriesDataOffset += 2;
					$atom_structure['sample_description_table'][$i]['data']             =                           substr($atom_data, $stsdEntriesDataOffset, ($atom_structure['sample_description_table'][$i]['size'] - 4 - 4 - 6 - 2));
					$stsdEntriesDataOffset += ($atom_structure['sample_description_table'][$i]['size'] - 4 - 4 - 6 - 2);

					$atom_structure['sample_description_table'][$i]['encoder_version']  = lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  0, 2));
					$atom_structure['sample_description_table'][$i]['encoder_revision'] = lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  2, 2));
					...
					switch ($atom_structure['sample_description_table'][$i]['encoder_vendor']) {
						...
						case "\x00\x00\x00\x00":
							// audio tracks
							$atom_structure['sample_description_table'][$i]['audio_channels']       =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  8,  2));
							$atom_structure['sample_description_table'][$i]['audio_bit_depth']      =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 10,  2));
							$atom_structure['sample_description_table'][$i]['audio_compression_id'] =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 12,  2));
							$atom_structure['sample_description_table'][$i]['audio_packet_size']    =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 14,  2));
							$atom_structure['sample_description_table'][$i]['audio_sample_rate']    = lib::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 16,  4));

							// video tracks
							// http://developer.apple.com/library/mac/#documentation/QuickTime/QTFF/QTFFChap3/qtff3.html
							$atom_structure['sample_description_table'][$i]['temporal_quality'] =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  8,  4));
							$atom_structure['sample_description_table'][$i]['spatial_quality']  =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 12,  4));
							$atom_structure['sample_description_table'][$i]['width']            =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 16,  2));
							$atom_structure['sample_description_table'][$i]['height']           =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 18,  2));
							$atom_structure['sample_description_table'][$i]['resolution_x']     = lib::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 24,  4));
							$atom_structure['sample_description_table'][$i]['resolution_y']     = lib::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 28,  4));
							$atom_structure['sample_description_table'][$i]['data_size']        =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 32,  4));
							$atom_structure['sample_description_table'][$i]['frame_count']      =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 36,  2));
							$atom_structure['sample_description_table'][$i]['compressor_name']  =                             substr($atom_structure['sample_description_table'][$i]['data'], 38,  4);
							$atom_structure['sample_description_table'][$i]['pixel_depth']      =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 42,  2));
							$atom_structure['sample_description_table'][$i]['color_table_id']   =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 44,  2));
	...
	$info['quicktime']['video']['resolution_y'] = $info['video']['resolution_y'];
						...
						default:
							switch ($atom_structure['sample_description_table'][$i]['data_format']) {
								...
								default:
									// video atom
									$atom_structure['sample_description_table'][$i]['video_temporal_quality']  =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  8,  4));
									$atom_structure['sample_description_table'][$i]['video_spatial_quality']   =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 12,  4));
									$atom_structure['sample_description_table'][$i]['video_frame_width']       =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 16,  2));
									$atom_structure['sample_description_table'][$i]['video_frame_height']      =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 18,  2));
									$atom_structure['sample_description_table'][$i]['video_resolution_x']      = lib::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 20,  4));
									$atom_structure['sample_description_table'][$i]['video_resolution_y']      = lib::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 24,  4));
									$atom_structure['sample_description_table'][$i]['video_data_size']         =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 28,  4));
									$atom_structure['sample_description_table'][$i]['video_frame_count']       =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 32,  2));
									$atom_structure['sample_description_table'][$i]['video_encoder_name_len']  =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 34,  1));
									$atom_structure['sample_description_table'][$i]['video_encoder_name']      =                             substr($atom_structure['sample_description_table'][$i]['data'], 35, $atom_structure['sample_description_table'][$i]['video_encoder_name_len']);
									$atom_structure['sample_description_table'][$i]['video_pixel_color_depth'] =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 66,  2));
									$atom_structure['sample_description_table'][$i]['video_color_table_id']    =   lib::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 68,  2));
			...
			case 'stts': // Sample Table Time-to-Sample atom
				...
				for ($i = 0; $i < $max_stts_entries_to_scan; $i++) {
					$atom_structure['time_to_sample_table'][$i]['sample_count']    = lib::BigEndian2Int(substr($atom_data, $sttsEntriesDataOffset, 4));
					$sttsEntriesDataOffset += 4;
					$atom_structure['time_to_sample_table'][$i]['sample_duration'] = lib::BigEndian2Int(substr($atom_data, $sttsEntriesDataOffset, 4));
					$sttsEntriesDataOffset += 4;
			...
			case 'stss': // Sample Table Sync Sample (key frames) atom
				if ($ParseAllPossibleAtoms) {
					...
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['time_to_sample_table'][$i] = lib::BigEndian2Int(substr($atom_data, $stssEntriesDataOffset, 4));
			...
			case 'stsc': // Sample Table Sample-to-Chunk atom
				if ($ParseAllPossibleAtoms) {
					...
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['sample_to_chunk_table'][$i]['first_chunk']        = lib::BigEndian2Int(substr($atom_data, $stscEntriesDataOffset, 4));
						$stscEntriesDataOffset += 4;
						$atom_structure['sample_to_chunk_table'][$i]['samples_per_chunk']  = lib::BigEndian2Int(substr($atom_data, $stscEntriesDataOffset, 4));
						$stscEntriesDataOffset += 4;
						$atom_structure['sample_to_chunk_table'][$i]['sample_description'] = lib::BigEndian2Int(substr($atom_data, $stscEntriesDataOffset, 4));
						$stscEntriesDataOffset += 4;
					}
				}
				break;
			...
			case 'stsz': // Sample Table SiZe atom
				if ($ParseAllPossibleAtoms) {
					$atom_structure['version']        = lib::BigEndian2Int(substr($atom_data,  0, 1));
					$atom_structure['flags_raw']      = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
					$atom_structure['sample_size']    = lib::BigEndian2Int(substr($atom_data,  4, 4));
					$atom_structure['number_entries'] = lib::BigEndian2Int(substr($atom_data,  8, 4));
					$stszEntriesDataOffset = 12;
					if ($atom_structure['sample_size'] == 0) {
						for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
							$atom_structure['sample_size_table'][$i] = lib::BigEndian2Int(substr($atom_data, $stszEntriesDataOffset, 4));
			...
			case 'stco': // Sample Table Chunk Offset atom
				if ($ParseAllPossibleAtoms) {
					...
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['chunk_offset_table'][$i] = lib::BigEndian2Int(substr($atom_data, $stcoEntriesDataOffset, 4));
			...
			case 'co64': // Chunk Offset 64-bit (version of "stco" that supports > 2GB files)
				if ($ParseAllPossibleAtoms) {
					...
					$stcoEntriesDataOffset = 8;
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['chunk_offset_table'][$i] = lib::BigEndian2Int(substr($atom_data, $stcoEntriesDataOffset, 8));
						$stcoEntriesDataOffset += 8;
					}
				}
				break;
			...
			case 'dref': // Data REFerence atom
				...
				$drefDataOffset = 8;
				for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
					$atom_structure['data_references'][$i]['size']                    = lib::BigEndian2Int(substr($atom_data, $drefDataOffset, 4));
					$drefDataOffset += 4;
					$atom_structure['data_references'][$i]['type']                    =                           substr($atom_data, $drefDataOffset, 4);
					$drefDataOffset += 4;
					$atom_structure['data_references'][$i]['version']                 = lib::BigEndian2Int(substr($atom_data,  $drefDataOffset, 1));
					$drefDataOffset += 1;
					$atom_structure['data_references'][$i]['flags_raw']               = lib::BigEndian2Int(substr($atom_data,  $drefDataOffset, 3)); // hardcoded: 0x0000
					$drefDataOffset += 3;
			...
			case 'gmin': // base Media INformation atom
				$atom_structure['version']                = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']              = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['graphics_mode']          = lib::BigEndian2Int(substr($atom_data,  4, 2));
				$atom_structure['opcolor_red']            = lib::BigEndian2Int(substr($atom_data,  6, 2));
				$atom_structure['opcolor_green']          = lib::BigEndian2Int(substr($atom_data,  8, 2));
				$atom_structure['opcolor_blue']           = lib::BigEndian2Int(substr($atom_data, 10, 2));
				$atom_structure['balance']                = lib::BigEndian2Int(substr($atom_data, 12, 2));
				$atom_structure['reserved']               = lib::BigEndian2Int(substr($atom_data, 14, 2));
				break;
			...
			case 'smhd': // Sound Media information HeaDer atom
				$atom_structure['version']                = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']              = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['balance']                = lib::BigEndian2Int(substr($atom_data,  4, 2));
				$atom_structure['reserved']               = lib::BigEndian2Int(substr($atom_data,  6, 2));
				break;
			...
			case 'mdhd': // MeDia HeaDer atom
				$atom_structure['version']               = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']             = lib::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['creation_time']         = lib::BigEndian2Int(substr($atom_data,  4, 4));
				$atom_structure['modify_time']           = lib::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['time_scale']            = lib::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['duration']              = lib::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['language_id']           = lib::BigEndian2Int(substr($atom_data, 20, 2));
				$atom_structure['quality']               = lib::BigEndian2Int(substr($atom_data, 22, 2));
				...
				$atom_structure['creation_time_unix']    = lib::DateMac2Unix($atom_structure['creation_time']);
				$atom_structure['modify_time_unix']      = lib::DateMac2Unix($atom_structure['modify_time']);
			...
			case 'pnot': // Preview atom
				$atom_structure['modification_date']      = lib::BigEndian2Int(substr($atom_data,  0, 4)); // "standard Macintosh format"
				$atom_structure['version_number']         = lib::BigEndian2Int(substr($atom_data,  4, 2)); // hardcoded: 0x00
				$atom_structure['atom_type']              =                           substr($atom_data,  6, 4);        // usually: 'PICT'
				$atom_structure['atom_index']             = lib::BigEndian2Int(substr($atom_data, 10, 2)); // usually: 0x01

				$atom_structure['modification_date_unix'] = lib::DateMac2Unix($atom_structure['modification_date']);
				break;


			case 'crgn': // Clipping ReGioN atom
				$atom_structure['region_size']   = lib::BigEndian2Int(substr($atom_data,  0, 2)); // The Region size, Region boundary box,
				$atom_structure['boundary_box']  = lib::BigEndian2Int(substr($atom_data,  2, 8)); // and Clipping region data fields
				$atom_structure['clipping_data'] =                           substr($atom_data, 10);           // constitute a QuickDraw region.
				break;


			case 'load': // track LOAD settings atom
				$atom_structure['preload_start_time'] = lib::BigEndian2Int(substr($atom_data,  0, 4));
				$atom_structure['preload_duration']   = lib::BigEndian2Int(substr($atom_data,  4, 4));
				$atom_structure['preload_flags_raw']  = lib::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['default_hints_raw']  = lib::BigEndian2Int(substr($atom_data, 12, 4));
			...
			case 'ssrc': // non-primary SouRCe atom
				for ($i = 0; $i < strlen($atom_data); $i += 4) {
					@$atom_structure['track_id'][] = lib::BigEndian2Int(substr($atom_data, $i, 4));
				}
				break;
			...
			case 'elst': // Edit LiST atom
				...
				$atom_structure['number_entries'] = lib::BigEndian2Int(substr($atom_data,  4, 4));
				for ($i = 0; $i < $atom_structure['number_entries']; $i++ ) {
					$atom_structure['edit_list'][$i]['track_duration'] =   lib::BigEndian2Int(substr($atom_data, 8 + ($i * 12) + 0, 4));
					$atom_structure['edit_list'][$i]['media_time']     =   lib::BigEndian2Int(substr($atom_data, 8 + ($i * 12) + 4, 4));
					$atom_structure['edit_list'][$i]['media_rate']     = lib::FixedPoint16_16(substr($atom_data, 8 + ($i * 12) + 8, 4));
				}
				break;
			...
			case 'ctab': // Color TABle atom
				$atom_structure['color_table_seed']   = lib::BigEndian2Int(substr($atom_data,  0, 4)); // hardcoded: 0x00000000
				$atom_structure['color_table_flags']  = lib::BigEndian2Int(substr($atom_data,  4, 2)); // hardcoded: 0x8000
				$atom_structure['color_table_size']   = lib::BigEndian2Int(substr($atom_data,  6, 2)) + 1;
				for ($colortableentry = 0; $colortableentry < $atom_structure['color_table_size']; $colortableentry++) {
					$atom_structure['color_table'][$colortableentry]['alpha'] = lib::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 0, 2));
					$atom_structure['color_table'][$colortableentry]['red']   = lib::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 2, 2));
					$atom_structure['color_table'][$colortableentry]['green'] = lib::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 4, 2));
					$atom_structure['color_table'][$colortableentry]['blue']  = lib::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 6, 2));
				}
				break;
			...
			case 'mvhd': // MoVie HeaDer atom
				...
				$atom_structure['time_scale']         =   lib::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['duration']           =   lib::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['preferred_rate']     = lib::FixedPoint16_16(substr($atom_data, 20, 4));
				$atom_structure['preferred_volume']   =   lib::FixedPoint8_8(substr($atom_data, 24, 2));
				$atom_structure['reserved']           =                             substr($atom_data, 26, 10);
				$atom_structure['matrix_a']           = lib::FixedPoint16_16(substr($atom_data, 36, 4));
				$atom_structure['matrix_b']           = lib::FixedPoint16_16(substr($atom_data, 40, 4));
				$atom_structure['matrix_u']           =  lib::FixedPoint2_30(substr($atom_data, 44, 4));
				$atom_structure['matrix_c']           = lib::FixedPoint16_16(substr($atom_data, 48, 4));
				$atom_structure['matrix_d']           = lib::FixedPoint16_16(substr($atom_data, 52, 4));
				$atom_structure['matrix_v']           =  lib::FixedPoint2_30(substr($atom_data, 56, 4));
				$atom_structure['matrix_x']           = lib::FixedPoint16_16(substr($atom_data, 60, 4));
				$atom_structure['matrix_y']           = lib::FixedPoint16_16(substr($atom_data, 64, 4));
				$atom_structure['matrix_w']           =  lib::FixedPoint2_30(substr($atom_data, 68, 4));
				$atom_structure['preview_time']       =   lib::BigEndian2Int(substr($atom_data, 72, 4));
				$atom_structure['preview_duration']   =   lib::BigEndian2Int(substr($atom_data, 76, 4));
				$atom_structure['poster_time']        =   lib::BigEndian2Int(substr($atom_data, 80, 4));
				$atom_structure['selection_time']     =   lib::BigEndian2Int(substr($atom_data, 84, 4));
				$atom_structure['selection_duration'] =   lib::BigEndian2Int(substr($atom_data, 88, 4));
				$atom_structure['current_time']       =   lib::BigEndian2Int(substr($atom_data, 92, 4));
				$atom_structure['next_track_id']      =   lib::BigEndian2Int(substr($atom_data, 96, 4));
			...
			case 'tkhd': // TracK HeaDer atom
				...
				$atom_structure['creation_time']       =   lib::BigEndian2Int(substr($atom_data,  4, 4));
				$atom_structure['modify_time']         =   lib::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['trackid']             =   lib::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['reserved1']           =   lib::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['duration']            =   lib::BigEndian2Int(substr($atom_data, 20, 4));
				$atom_structure['reserved2']           =   lib::BigEndian2Int(substr($atom_data, 24, 8));
				$atom_structure['layer']               =   lib::BigEndian2Int(substr($atom_data, 32, 2));
				$atom_structure['alternate_group']     =   lib::BigEndian2Int(substr($atom_data, 34, 2));
				$atom_structure['volume']              =   lib::FixedPoint8_8(substr($atom_data, 36, 2));
				$atom_structure['reserved3']           =   lib::BigEndian2Int(substr($atom_data, 38, 2));
// http://developer.apple.com/library/mac/#documentation/QuickTime/RM/MovieBasics/MTEditing/K-Chapter/11MatrixFunctions.html
// http://developer.apple.com/library/mac/#documentation/QuickTime/qtff/QTFFChap4/qtff4.html#//apple_ref/doc/uid/TP40000939-CH206-18737
				$atom_structure['matrix_a']            = lib::FixedPoint16_16(substr($atom_data, 40, 4));
				$atom_structure['matrix_b']            = lib::FixedPoint16_16(substr($atom_data, 44, 4));
				$atom_structure['matrix_u']            =  lib::FixedPoint2_30(substr($atom_data, 48, 4));
				$atom_structure['matrix_c']            = lib::FixedPoint16_16(substr($atom_data, 52, 4));
				$atom_structure['matrix_d']            = lib::FixedPoint16_16(substr($atom_data, 56, 4));
				$atom_structure['matrix_v']            =  lib::FixedPoint2_30(substr($atom_data, 60, 4));
				$atom_structure['matrix_x']            = lib::FixedPoint16_16(substr($atom_data, 64, 4));
				$atom_structure['matrix_y']            = lib::FixedPoint16_16(substr($atom_data, 68, 4));
				$atom_structure['matrix_w']            =  lib::FixedPoint2_30(substr($atom_data, 72, 4));
				$atom_structure['width']               = lib::FixedPoint16_16(substr($atom_data, 76, 4));
				$atom_structure['height']              = lib::FixedPoint16_16(substr($atom_data, 80, 4));
			...
			case 'iods': // Initial Object DeScriptor atom
				...
				$offset = 0;
				$atom_structure['version']                =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['flags_raw']              =       lib::BigEndian2Int(substr($atom_data, $offset, 3));
				$offset += 3;
				$atom_structure['mp4_iod_tag']            =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['length']                 = $this->quicktime_read_mp4_descr_length($atom_data, $offset);
				//$offset already adjusted by quicktime_read_mp4_descr_length()
				$atom_structure['object_descriptor_id']   =       lib::BigEndian2Int(substr($atom_data, $offset, 2));
				$offset += 2;
				$atom_structure['od_profile_level']       =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['scene_profile_level']    =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['audio_profile_id']       =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['video_profile_id']       =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['graphics_profile_level'] =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				...
				for ($i = 0; $i < $atom_structure['num_iods_tracks']; $i++) {
					$atom_structure['track'][$i]['ES_ID_IncTag'] =       lib::BigEndian2Int(substr($atom_data, $offset, 1));
					$offset += 1;
					$atom_structure['track'][$i]['length']       = $this->quicktime_read_mp4_descr_length($atom_data, $offset);
					//$offset already adjusted by quicktime_read_mp4_descr_length()
					$atom_structure['track'][$i]['track_id']     =       lib::BigEndian2Int(substr($atom_data, $offset, 4));
					$offset += 4;
				}
			...
			case 'ftyp': // FileTYPe (?) atom (for MP4 it seems)
				...
				$atom_structure['unknown_1'] = lib::BigEndian2Int(substr($atom_data,  4, 4));
...
/* due to lack of known documentation, this is a kludge implementation. If you know of documentation on how mdat is properly structed, please send it to info@getid3.org */
				...
				while (($mdat_offset < (strlen($atom_data) - 8))
					&& ($chapter_string_length = lib::BigEndian2Int(substr($atom_data, $mdat_offset, 2)))
				...
				if (($atomsize > 8) && (!isset($info['avdataend_tmp']) || ($info['quicktime'][$atomname]['size'] > ($info['avdataend_tmp'] - $info['avdataoffset'])))) {
					...
					$mp3 = new mp3($getid3_temp);
					if ($mp3->MPEGaudioHeaderValid($mp3->MPEGaudioHeaderDecode($this->fread(4)))) {
						$mp3->getOnlyMPEGaudioInfo($getid3_temp->info['avdataoffset'], false);
			...
			case 'nsav': // NoSAVe atom
				...
				$atom_structure['data'] = lib::BigEndian2Int(substr($atom_data,  0, 4));
				break;
			...
			case 'pano': // PANOrama track (seen on QTVR)
				$atom_structure['pano'] = lib::BigEndian2Int(substr($atom_data,  0, 4));
				break;
			...
			case 'imgt': // IMaGe Track reference (kQTVRImageTrackRefType) (seen on QTVR)
				for ($i = 0; $i < ($atom_structure['size'] - 8); $i += 4) {
					$atom_structure['imgt'][] = lib::BigEndian2Int(substr($atom_data, $i, 4));
				}
				break;
			...
			case 'data': // metaDATA atom
				...
				$atom_structure['unknown']  = lib::BigEndian2Int(substr($atom_data, 4 + 2, 2));
			...
			case 'keys': // KEYS that may be present in the metadata atom.
				...
				$atom_structure['version']       = lib::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']     = lib::BigEndian2Int(substr($atom_data,  1, 3));
				$atom_structure['entry_count']   = lib::BigEndian2Int(substr($atom_data,  4, 4));
				$keys_atom_offset = 8;
				for ($i = 1; $i <= $atom_structure['entry_count']; $i++) {
					$atom_structure['keys'][$i]['key_size']      = lib::BigEndian2Int(substr($atom_data, $keys_atom_offset + 0, 4));
			...
			case 'loci':// 3GP location (El Loco)
				$loffset = 0;
				$info['quicktime']['comments']['gps_flags']     =   lib::BigEndian2Int(substr($atom_data, 0, 4));
				$info['quicktime']['comments']['gps_lang']      =   lib::BigEndian2Int(substr($atom_data, 4, 2));
				$info['quicktime']['comments']['gps_location']  =           $this->LociString(substr($atom_data, 6), $loffset);
				$loci_data = substr($atom_data, 6 + $loffset);
				$info['quicktime']['comments']['gps_role']      =   lib::BigEndian2Int(substr($loci_data, 0, 1));
				$info['quicktime']['comments']['gps_longitude'] = lib::FixedPoint16_16(substr($loci_data, 1, 4));
				$info['quicktime']['comments']['gps_latitude']  = lib::FixedPoint16_16(substr($loci_data, 5, 4));
				$info['quicktime']['comments']['gps_altitude']  = lib::FixedPoint16_16(substr($loci_data, 9, 4));
			...
			case 'chpl': // CHaPter List
				...
				$chpl_version = lib::BigEndian2Int(substr($atom_data, 4, 1)); // Expected to be 0
				$chpl_flags   = lib::BigEndian2Int(substr($atom_data, 5, 3)); // Reserved, set to 0
				$chpl_count   = lib::BigEndian2Int(substr($atom_data, 8, 1));
				$chpl_offset = 9;
				for ($i = 0; $i < $chpl_count; $i++) {
					...
					$info['quicktime']['chapters'][$i]['timestamp'] = lib::BigEndian2Int(substr($atom_data, $chpl_offset, 8)) / 10000000; // timestamps are stored as 100-nanosecond units
					$chpl_offset += 8;
					$chpl_title_size = lib::BigEndian2Int(substr($atom_data, $chpl_offset, 1));
					$chpl_offset += 1;
			...
			default:
				$this->warning('Unknown QuickTime atom type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $atomname).'" ('.trim(lib::PrintHexBytes($atomname)).') at offset '.$baseoffset);
				$atom_structure['data'] = $atom_data;
				break;
		}
...
//echo 'QuicktimeParseContainerAtom('.substr($atom_data, 4, 4).') @ '.$baseoffset.'<br><br>';
		...
		$subatomoffset  = 0;
		$subatomcounter = 0;
		if ((strlen($atom_data) == 4) && (lib::BigEndian2Int($atom_data) == 0x00000000)) {
			return false;
		}
		while ($subatomoffset < strlen($atom_data)) {
			$subatomsize = lib::BigEndian2Int(substr($atom_data, $subatomoffset + 0, 4));
...
//echo lib::PrintHexBytes(substr($atom_data, $offset, 4)).'<br>';
			$record_type       = lib::BigEndian2Int(substr($atom_data, $offset, 4));  $offset += 4;
			$data_size_type    = lib::BigEndian2Int(substr($atom_data, $offset, 2));  $offset += 2;
			$data_size         = lib::BigEndian2Int(substr($atom_data, $offset, 2));  $offset += 2;
			switch ($data_size_type) {
				case 0x0001: // 0x0001 = flag   (size field *= 1-byte)
					$data = lib::BigEndian2Int(substr($atom_data, $offset, $data_size * 1));
					$offset += ($data_size * 1);
					break;
				...
				case 0x0005: // 0x0005 = float  (size field *= 8-byte), values are stored aaaabbbb where value is aaaa/bbbb; possibly multiple sets of values appended together
					...
					for ($i = 0; $i < $data_size; $i++) {
						$numerator    = lib::BigEndian2Int(substr($atom_data, $offset + ($i * 8) + 0, 4));
						$denomninator = lib::BigEndian2Int(substr($atom_data, $offset + ($i * 8) + 4, 4));
						if ($denomninator == 0) {
							$data[$i] = false;
						} else {
...
echo 'QuicktimeParseNikonNCTG()::unknown $data_size_type: '.$data_size_type.'<br>';
			...
			switch ($record_type) {
				...
				case 0x02000024: // WorldTime
					...
					$timezone = lib::BigEndian2Int(substr($data, 0, 2));
					if ($timezone & 0x8000) {
						$timezone = 0 - (0x10000 - $timezone);
					}
					$timezone /= 60;

					$dst = (bool) lib::BigEndian2Int(substr($data, 2, 1));
					switch (lib::BigEndian2Int(substr($data, 3, 1))) {
						case 2:
    ...
    public function LociString($lstring, &$count) {
            ...
            if ($len > 2 && (($lstring[0] == "\xFE" && $lstring[1] == "\xFF") || ($lstring[0] == "\xFF" && $lstring[1] == "\xFE"))) {
                ...
                if (preg_match('/(.*)\x00/', $lstring, $lmatches)){
                    ...
                    return lib::iconv_fallback_utf16_utf8($lmatches[1]);
                } else {
                    return '';
                }
            } else {
