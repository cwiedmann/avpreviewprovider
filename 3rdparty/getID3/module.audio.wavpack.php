...
class wavpack extends handler
{
	...
	public function Analyze() {
		...
		while (true) {
			...
			$blockheader_size   = lib::LittleEndian2Int(substr($wavpackheader,  4,  4));

			$magic = 'wvpk';
			if ($blockheader_magic != $magic) {
				$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$blockheader_offset.', found "'.lib::PrintHexBytes($blockheader_magic).'"');
			...
			if (empty($info['wavpack']['blockheader']['block_samples']) ||
				...
				$info['wavpack']['blockheader']['total_samples'] = lib::LittleEndian2Int(substr($wavpackheader, 12,  4));
				$info['wavpack']['blockheader']['block_index']   = lib::LittleEndian2Int(substr($wavpackheader, 16,  4));
				$info['wavpack']['blockheader']['block_samples'] = lib::LittleEndian2Int(substr($wavpackheader, 20,  4));
				$info['wavpack']['blockheader']['flags_raw']     = lib::LittleEndian2Int(substr($wavpackheader, 24,  4));
				$info['wavpack']['blockheader']['crc']           = lib::LittleEndian2Int(substr($wavpackheader, 28,  4));
			...
			while (!feof($this->getid3->fp) && ($this->ftell() < ($blockheader_offset + $blockheader_size + 8))) {
				...
				$metablock['size'] = lib::LittleEndian2Int(substr($metablockheader, 1)) * 2; // size is stored in words
				$metablock['data'] = null;
				...
				if ($metablock['size'] > 0) {
					...
					switch ($metablock['function_id']) {
						case 0x21: // ID_RIFF_HEADER
							...
							$original_wav_filesize = lib::LittleEndian2Int(substr($metablock['data'], 4, 4));

							$getid3_temp = new getID3();
							$getid3_temp->openfile($this->getid3->filename);
							$riff = new riff($getid3_temp);
							$riff->ParseRIFFdata($metablock['data']);
						...
						case 0x22: // ID_RIFF_TRAILER
							...
							$riff = new riff($getid3_temp);
							$metablock['riff'] = $riff->ParseRIFF($startoffset, $startoffset + $metablock['size']);
						...
						case 0x25: // ID_CONFIG_BLOCK
							$metablock['flags_raw'] = lib::LittleEndian2Int(substr($metablock['data'], 0, 3));
						...
						case 0x26: // ID_MD5_CHECKSUM
							if (strlen($metablock['data']) == 16) {
								$info['md5_data_source'] = strtolower(lib::PrintHexBytes($metablock['data'], true, false, false));
							} else {
