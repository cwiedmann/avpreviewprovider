	...
	public function Analyze() {
		...
		switch (substr($rawdata, $offset, 4)) {
			...
			case 'LA04':
				...
				$info['la']['uncompressed_size'] = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				$offset += 4;
				...
				if ($WAVEchunk !== 'WAVE') {
					$this->error('Expected "WAVE" ('.lib::PrintHexBytes('WAVE').') at offset '.$offset.', found "'.$WAVEchunk.'" ('.lib::PrintHexBytes($WAVEchunk).') instead.');
					return false;
				}
				$offset += 4;
				...
				if ($info['la']['version'] >= 0.3) {
					...
					$info['la']['fmt_size']    = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				...
				if ($fmt_chunk !== 'fmt ') {
					$this->error('Expected "fmt " ('.lib::PrintHexBytes('fmt ').') at offset '.$offset.', found "'.$fmt_chunk.'" ('.lib::PrintHexBytes($fmt_chunk).') instead.');
					return false;
				}
				$offset += 4;
				$fmt_size = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				$offset += 4;

				$info['la']['raw']['format']  = lib::LittleEndian2Int(substr($rawdata, $offset, 2));
				$offset += 2;

				$info['la']['channels']       = lib::LittleEndian2Int(substr($rawdata, $offset, 2));
				$offset += 2;
				...
				$info['la']['sample_rate'] = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				$offset += 4;
				...
				$info['la']['bytes_per_second']     = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				$offset += 4;
				$info['la']['bytes_per_sample']     = lib::LittleEndian2Int(substr($rawdata, $offset, 2));
				$offset += 2;
				$info['la']['bits_per_sample']      = lib::LittleEndian2Int(substr($rawdata, $offset, 2));
				$offset += 2;

				$info['la']['samples']              = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				$offset += 4;

				$info['la']['raw']['flags']         = lib::LittleEndian2Int(substr($rawdata, $offset, 1));
				$offset += 1;
				...
				$info['la']['original_crc']         = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
				$offset += 4;
				...
				if ($info['la']['flags']['seekable']) {
					...
					for ($i = 0; $i < $info['la']['seekpoint_count']; $i++) {
						$info['la']['seekpoints'][] = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
						$offset += 4;
					}
				}
				...
				if ($info['la']['version'] >= 0.3) {
					...
					$info['la']['footerstart'] = lib::LittleEndian2Int(substr($rawdata, $offset, 4));
					$offset += 4;
				...
				if ($info['la']['footerstart'] < $info['avdataend']) {
					if ($RIFFtempfilename = tempnam(AVPREVIEWPROVIDERGETID3_TEMP_DIR, 'id3')) {
						if ($RIFF_fp = fopen($RIFFtempfilename, 'w+b')) {
							...
							$RIFFdata = 'RIFF'.lib::LittleEndian2String(strlen($RIFFdata), 4, false).$RIFFdata;
