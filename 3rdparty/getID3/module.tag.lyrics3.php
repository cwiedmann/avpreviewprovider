...
class lyrics3 extends handler
{
		...
		if (isset($lyrics3offset)) {
			...
			if (!isset($info['ape'])) {
				if (isset($info['lyrics3']['tag_offset_start'])) {
					$AVPREVIEWPROVIDERGETID3_ERRORARRAY = &$info['warning'];
					lib::IncludeDependency(AVPREVIEWPROVIDERGETID3_INCLUDEPATH.'module.tag.apetag.php', __FILE__, true);
					$getid3_temp = new getID3();
					$getid3_temp->openfile($this->getid3->filename);
					$apetag = new apetag($getid3_temp);
					$apetag->overrideendoffset = $info['lyrics3']['tag_offset_start'];
					$apetag->Analyze();
	...
	public function getLyrics3Data($endoffset, $version, $length) {
		...
		if (!lib::intValueSupported($endoffset)) {
			$this->warning('Unable to check for Lyrics3 because file is larger than '.round(PHP_INT_MAX / 1073741824).'GB');
			return false;
		}
