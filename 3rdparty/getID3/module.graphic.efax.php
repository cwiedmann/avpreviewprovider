...
class efax extends handler
{
	...
	public function Analyze() {
		...
		if ($info['efax']['header']['magic'] != "\xDC\xFE") {
			$this->error('Invalid eFax byte order identifier (expecting DC FE, found '.lib::PrintHexBytes($info['efax']['header']['magic']).') at offset '.$info['avdataoffset']);
			return false;
		}
		$info['fileformat'] = 'efax';

		$info['efax']['header']['filesize'] = lib::LittleEndian2Int(substr($efaxheader, 2, 4));
		...
		$info['efax']['header']['pages']      = lib::LittleEndian2Int(substr($efaxheader, 198, 2));
		$info['efax']['header']['data_bytes'] = lib::LittleEndian2Int(substr($efaxheader, 202, 4));
