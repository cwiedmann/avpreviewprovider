...
class szip extends handler
{
	...
	public function Analyze() {
		...
		if (substr($SZIPHeader, 0, 4) != "SZ\x0A\x04") {
			$this->error('Expecting "53 5A 0A 04" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes(substr($SZIPHeader, 0, 4)).'"');
			return false;
		}
		$info['fileformat']            = 'szip';
		$info['szip']['major_version'] = lib::BigEndian2Int(substr($SZIPHeader, 4, 1));
		$info['szip']['minor_version'] = lib::BigEndian2Int(substr($SZIPHeader, 5, 1));
...
return false;
		...
		while (!$this->feof()) {
			...
			switch ($NextBlockID) {
				...
				case 'BH':
					$BHheaderbytes  = lib::BigEndian2Int($this->fread(3));
					...
					while (strpos($BHheaderdata, "\x00", $BHheaderoffset) > 0) {
						...
						$BHdataArray['filelength'] = lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset, 3));
						$BHheaderoffset += 3;

						$BHdataArray['access_flags'] = lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset, 2));
						$BHheaderoffset += 2;

						$BHdataArray['creation_time'] = lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset, 4));
						$BHheaderoffset += 4;

						$BHdataArray['modification_time'] = lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset, 4));
						$BHheaderoffset += 4;

						$BHdataArray['access_time'] = lib::BigEndian2Int(substr($BHheaderdata, $BHheaderoffset, 4));
						$BHheaderoffset += 4;
