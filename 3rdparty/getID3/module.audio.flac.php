...
class flac extends handler
{
	...
	public function Analyze() {
		...
		if ($StreamMarker != self::syncword) {
			return $this->error('Expecting "'.lib::PrintHexBytes(self::syncword).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($StreamMarker).'"');
		}
	...
	public function parseMETAdata() {
		...
		do {
			...
			$LBFBT         = lib::BigEndian2Int(substr($BlockHeader, 0, 1));
			$LastBlockFlag = (bool) ($LBFBT & 0x80);
			$BlockType     =        ($LBFBT & 0x7F);
			$BlockLength   = lib::BigEndian2Int(substr($BlockHeader, 1, 3));
	...
	private function parseSTREAMINFO($BlockData) {
		...
		$streaminfo['min_block_size']  = lib::BigEndian2Int(substr($BlockData, 0, 2));
		$streaminfo['max_block_size']  = lib::BigEndian2Int(substr($BlockData, 2, 2));
		$streaminfo['min_frame_size']  = lib::BigEndian2Int(substr($BlockData, 4, 3));
		$streaminfo['max_frame_size']  = lib::BigEndian2Int(substr($BlockData, 7, 3));

		$SRCSBSS                       = lib::BigEndian2Bin(substr($BlockData, 10, 8));
		$streaminfo['sample_rate']     = lib::Bin2Dec(substr($SRCSBSS,  0, 20));
		$streaminfo['channels']        = lib::Bin2Dec(substr($SRCSBSS, 20,  3)) + 1;
		$streaminfo['bits_per_sample'] = lib::Bin2Dec(substr($SRCSBSS, 23,  5)) + 1;
		$streaminfo['samples_stream']  = lib::Bin2Dec(substr($SRCSBSS, 28, 36));
	...
	private function parseAPPLICATION($BlockData) {
		...
		$ApplicationID = lib::BigEndian2Int(substr($BlockData, 0, 4));
	...
	private function parseSEEKTABLE($BlockData) {
		...
		while ($offset < $BlockLength) {
			...
			if ($SampleNumberString == $placeholderpattern) {
				...
				// placeholder point
				lib::safe_inc($info['flac']['SEEKTABLE']['placeholders'], 1);
				$offset += 10;

			} else {

				$SampleNumber                                        = lib::BigEndian2Int($SampleNumberString);
				$info['flac']['SEEKTABLE'][$SampleNumber]['offset']  = lib::BigEndian2Int(substr($BlockData, $offset, 8));
				$offset += 8;
				$info['flac']['SEEKTABLE'][$SampleNumber]['samples'] = lib::BigEndian2Int(substr($BlockData, $offset, 2));
				$offset += 2;
	...
	private function parseVORBIS_COMMENT($BlockData) {
		...
		$ogg = new ogg($this->getid3);
		if ($this->isDependencyFor('matroska')) {
			$ogg->setStringMode($this->data_string);
		}
		$ogg->ParseVorbisComments();
		if (isset($info['ogg'])) {
	...
	private function parseCUESHEET($BlockData) {
		...
		$offset += 128;
		$info['flac']['CUESHEET']['lead_in_samples']      =         lib::BigEndian2Int(substr($BlockData, $offset, 8));
		$offset += 8;
		$info['flac']['CUESHEET']['flags']['is_cd']       = (bool) (lib::BigEndian2Int(substr($BlockData, $offset, 1)) & 0x80);
		$offset += 1;

		$offset += 258; // reserved

		$info['flac']['CUESHEET']['number_tracks']        =         lib::BigEndian2Int(substr($BlockData, $offset, 1));
		$offset += 1;

		for ($track = 0; $track < $info['flac']['CUESHEET']['number_tracks']; $track++) {
			$TrackSampleOffset = lib::BigEndian2Int(substr($BlockData, $offset, 8));
			$offset += 8;
			$TrackNumber       = lib::BigEndian2Int(substr($BlockData, $offset, 1));
			$offset += 1;
			...
			$TrackFlagsRaw                                                             = lib::BigEndian2Int(substr($BlockData, $offset, 1));
			$offset += 1;
			...
			$info['flac']['CUESHEET']['tracks'][$TrackNumber]['index_points']          = lib::BigEndian2Int(substr($BlockData, $offset, 1));
			$offset += 1;

			for ($index = 0; $index < $info['flac']['CUESHEET']['tracks'][$TrackNumber]['index_points']; $index++) {
				$IndexSampleOffset = lib::BigEndian2Int(substr($BlockData, $offset, 8));
				$offset += 8;
				$IndexNumber       = lib::BigEndian2Int(substr($BlockData, $offset, 1));
				$offset += 1;
	...
	*/
	public function parsePICTURE() {
		...
		$picture['typeid']         = lib::BigEndian2Int($this->fread(4));
		$picture['picturetype']    = self::pictureTypeLookup($picture['typeid']);
		$picture['image_mime']     = $this->fread(lib::BigEndian2Int($this->fread(4)));
		$descr_length              = lib::BigEndian2Int($this->fread(4));
		if ($descr_length) {
			$picture['description'] = $this->fread($descr_length);
		}
		$picture['image_width']    = lib::BigEndian2Int($this->fread(4));
		$picture['image_height']   = lib::BigEndian2Int($this->fread(4));
		$picture['color_depth']    = lib::BigEndian2Int($this->fread(4));
		$picture['colors_indexed'] = lib::BigEndian2Int($this->fread(4));
		$picture['datalength']     = lib::BigEndian2Int($this->fread(4));
