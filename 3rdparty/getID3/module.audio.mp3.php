...
// mpeg-audio streams
define('AVPREVIEWPROVIDERGETID3_MP3_VALID_CHECK_FRAMES', 35);
...
class mp3 extends handler
{
	...
	public function decodeMPEGaudioHeader($offset, &$info, $recursivesearch=true, $ScanAsCBR=false, $FastMPEGheaderScan=false) {
		...
		$head4_key = lib::PrintHexBytes($head4, true, false, false);
		...
		if ($MPEGaudioHeaderValidCache[$head4_key]) {
			...
			$this->error('Invalid MPEG audio header ('.lib::PrintHexBytes($head4).') at offset '.$offset);
			return false;
		}
		...
		if (!$FastMPEGheaderScan) {
			...
			if ($thisfile_mpeg_audio['protection']) {
				$thisfile_mpeg_audio['crc'] = lib::BigEndian2Int(substr($headerstring, 4, 2));
			}
		}
		...
		if (substr($headerstring, 4 + 32, 4) == 'VBRI') {
			...
			$thisfile_mpeg_audio['VBR_encoder_version']     = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset +  4, 2)); // VbriVersion
			$thisfile_mpeg_audio['VBR_encoder_delay']       = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset +  6, 2)); // VbriDelay
			$thisfile_mpeg_audio['VBR_quality']             = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset +  8, 2)); // VbriQuality
			$thisfile_mpeg_audio['VBR_bytes']               = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 10, 4)); // VbriStreamBytes
			$thisfile_mpeg_audio['VBR_frames']              = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 14, 4)); // VbriStreamFrames
			$thisfile_mpeg_audio['VBR_seek_offsets']        = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 18, 2)); // VbriTableSize
			$thisfile_mpeg_audio['VBR_seek_scale']          = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 20, 2)); // VbriTableScale
			$thisfile_mpeg_audio['VBR_entry_bytes']         = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 22, 2)); // VbriEntryBytes
			$thisfile_mpeg_audio['VBR_entry_frames']        = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 24, 2)); // VbriEntryFrames

			$ExpectedNumberOfAudioBytes = $thisfile_mpeg_audio['VBR_bytes'];

			$previousbyteoffset = $offset;
			for ($i = 0; $i < $thisfile_mpeg_audio['VBR_seek_offsets']; $i++) {
				$Fraunhofer_OffsetN = lib::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset, $thisfile_mpeg_audio['VBR_entry_bytes']));
...
//				}
				...
				$thisfile_mpeg_audio['xing_flags_raw'] = lib::BigEndian2Int(substr($headerstring, $VBRidOffset + 4, 4));
				...
				if ($thisfile_mpeg_audio['xing_flags']['frames']) {
					$thisfile_mpeg_audio['VBR_frames'] = lib::BigEndian2Int(substr($headerstring, $VBRidOffset +  8, 4));
					//$thisfile_mpeg_audio['VBR_frames']--; // don't count header Xing/Info frame
				}
				if ($thisfile_mpeg_audio['xing_flags']['bytes']) {
					$thisfile_mpeg_audio['VBR_bytes']  = lib::BigEndian2Int(substr($headerstring, $VBRidOffset + 12, 4));
				}
				...
				if ($thisfile_mpeg_audio['xing_flags']['vbr_scale']) {
					$thisfile_mpeg_audio['VBR_scale'] = lib::BigEndian2Int(substr($headerstring, $VBRidOffset + 116, 4));
				}
				...
				if (substr($headerstring, $VBRidOffset + 120, 4) == 'LAME') {
					...
					if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.90') {
						...
						$thisfile_mpeg_audio_lame['vbr_quality'] = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0x9B, 1));

						// bytes $9C-$A4  Encoder short VersionString
						$thisfile_mpeg_audio_lame['short_version'] = substr($headerstring, $LAMEtagOffsetContant + 0x9C, 9);

						// byte $A5  Info Tag revision + VBR method
						$LAMEtagRevisionVBRmethod = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xA5, 1));
						...
						$thisfile_mpeg_audio_lame['lowpass_frequency'] = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xA6, 1)) * 100;
						...
						if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.94b') {
							...
							$thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] = (float) ((lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xA7, 4))) / 8388608);
						} else {
							// LAME 3.94a15 and earlier - 32-bit floating point
							// Actually 3.94a16 will fall in here too and be WRONG, but is hard to detect 3.94a16 vs 3.94a15
							$thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] = lib::LittleEndian2Float(substr($headerstring, $LAMEtagOffsetContant + 0xA7, 4));
						}
						if ($thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] == 0) {
							unset($thisfile_mpeg_audio_lame_RGAD['peak_amplitude']);
						} else {
							$thisfile_mpeg_audio_lame_RGAD['peak_db'] = lib::RGADamplitude2dB($thisfile_mpeg_audio_lame_RGAD['peak_amplitude']);
						}

						$thisfile_mpeg_audio_lame_raw['RGAD_track']      =   lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xAB, 2));
						$thisfile_mpeg_audio_lame_raw['RGAD_album']      =   lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xAD, 2));
						...
						if ($thisfile_mpeg_audio_lame_raw['RGAD_track'] != 0) {
							...
							$thisfile_mpeg_audio_lame_RGAD_track['name']       = lib::RGADnameLookup($thisfile_mpeg_audio_lame_RGAD_track['raw']['name']);
							$thisfile_mpeg_audio_lame_RGAD_track['originator'] = lib::RGADoriginatorLookup($thisfile_mpeg_audio_lame_RGAD_track['raw']['originator']);
							$thisfile_mpeg_audio_lame_RGAD_track['gain_db']    = lib::RGADadjustmentLookup($thisfile_mpeg_audio_lame_RGAD_track['raw']['gain_adjust'], $thisfile_mpeg_audio_lame_RGAD_track['raw']['sign_bit']);
						...
						if ($thisfile_mpeg_audio_lame_raw['RGAD_album'] != 0) {
							...
							$thisfile_mpeg_audio_lame_RGAD_album['name']       = lib::RGADnameLookup($thisfile_mpeg_audio_lame_RGAD_album['raw']['name']);
							$thisfile_mpeg_audio_lame_RGAD_album['originator'] = lib::RGADoriginatorLookup($thisfile_mpeg_audio_lame_RGAD_album['raw']['originator']);
							$thisfile_mpeg_audio_lame_RGAD_album['gain_db']    = lib::RGADadjustmentLookup($thisfile_mpeg_audio_lame_RGAD_album['raw']['gain_adjust'], $thisfile_mpeg_audio_lame_RGAD_album['raw']['sign_bit']);
						...
						$EncodingFlagsATHtype = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xAF, 1));
						...
						$thisfile_mpeg_audio_lame['raw']['abrbitrate_minbitrate'] = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB0, 1));
						...
						$EncoderDelays = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB1, 3));
						...
						// byte $B4  Misc
						$MiscByte = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB4, 1));
						...
						// byte $B5  MP3 Gain
						$thisfile_mpeg_audio_lame_raw['mp3_gain'] = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB5, 1), false, true);
						$thisfile_mpeg_audio_lame['mp3_gain_db']     = (lib::RGADamplitude2dB(2) / 4) * $thisfile_mpeg_audio_lame_raw['mp3_gain'];
						$thisfile_mpeg_audio_lame['mp3_gain_factor'] = pow(2, ($thisfile_mpeg_audio_lame['mp3_gain_db'] / 6));

						// bytes $B6-$B7  Preset and surround info
						$PresetSurroundBytes = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB6, 2));
						...
						$thisfile_mpeg_audio_lame['audio_bytes'] = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xB8, 4));
						...
						$thisfile_mpeg_audio_lame['music_crc']    = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xBC, 2));

						// bytes $BE-$BF  CRC-16 of Info Tag
						$thisfile_mpeg_audio_lame['lame_tag_crc'] = lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xBE, 2));
		...
		//
		//    $SideInfoBitstream = lib::BigEndian2Bin($SideInfoData);
		//    $SideInfoOffset = 0;
		//
	...
	public function RecursiveFrameScanning(&$offset, &$nextframetestoffset, $ScanAsCBR) {
		...
		for ($i = 0; $i < AVPREVIEWPROVIDERGETID3_MP3_VALID_CHECK_FRAMES; $i++) {
			// check next AVPREVIEWPROVIDERGETID3_MP3_VALID_CHECK_FRAMES frames for validity, to make sure we haven't run across a false synch
	...
	public function FreeFormatFrameLength($offset, $deepscan=false) {
		...
		if (!$framelength) {
			...
			if (!$framelength) {
				$this->error('Cannot find next free-format synch pattern ('.lib::PrintHexBytes($SyncPattern1).' or '.lib::PrintHexBytes($SyncPattern2).') after offset '.$offset);
				return false;
			} else {
	...
	public function getOnlyMPEGaudioInfoBruteForce() {
				...
				if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
					...
					if ($next4{0} == "\xFF") {
						...
						if ($MPEGaudioHeaderValidCache[$next4]) {
							...
							lib::safe_inc($Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]]);
							lib::safe_inc($Distribution['layer'][$LongMPEGlayerLookup[$head4]]);
							lib::safe_inc($Distribution['version'][$LongMPEGversionLookup[$head4]]);
							lib::safe_inc($Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])]);
							lib::safe_inc($Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]]);
		...
		$info['mpeg']['audio']['sample_rate']  = lib::array_max($Distribution['frequency'], true);

		$info['audio']['bitrate']      = $info['mpeg']['audio']['bitrate'];
		$info['audio']['bitrate_mode'] = $info['mpeg']['audio']['bitrate_mode'];
		$info['audio']['sample_rate']  = $info['mpeg']['audio']['sample_rate'];
		$info['audio']['dataformat']   = 'mp'.lib::array_max($Distribution['layer'], true);
		$info['fileformat']            = $info['audio']['dataformat'];
			...
			if (($header{$SynchSeekOffset} == "\xFF") && ($header{($SynchSeekOffset + 1)} > "\xE0")) { // synch detected
				...
				if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
					...
					if (isset($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode']) && ($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode'] == 'vbr')) {
						if (!(abs($info['audio']['bitrate'] - $FirstFrameThisfileInfo['audio']['bitrate']) <= 1)) {
							...
							if ($this->decodeMPEGaudioHeader($GarbageOffsetEnd, $dummy, true, true)) {
								...
								$this->warning('apparently-valid VBR header not used because could not find '.AVPREVIEWPROVIDERGETID3_MP3_VALID_CHECK_FRAMES.' consecutive MPEG-audio frames immediately after VBR header (garbage data for '.($GarbageOffsetEnd - $GarbageOffsetStart).' bytes between '.$GarbageOffsetStart.' and '.$GarbageOffsetEnd.'), but did find valid CBR stream starting at '.$GarbageOffsetEnd);
							} else {
								$this->warning('using data from VBR header even though could not find '.AVPREVIEWPROVIDERGETID3_MP3_VALID_CHECK_FRAMES.' consecutive MPEG-audio frames immediately after VBR header (garbage data for '.($GarbageOffsetEnd - $GarbageOffsetStart).' bytes between '.$GarbageOffsetStart.' and '.$GarbageOffsetEnd.')');
							}
						}
					}
					...
					if ($BitrateHistogram) {
						...
						for ($current_segment = 0; $current_segment < $max_scan_segments; $current_segment++) {
							...
							while ($this->decodeMPEGaudioHeader($synchstartoffset, $dummy, false, false, $FastMode)) {
								...
								if (empty($dummy['mpeg']['audio']['framelength'])) {
									...
									lib::safe_inc($info['mpeg']['audio']['bitrate_distribution'][$thisframebitrate]);
									lib::safe_inc($info['mpeg']['audio']['stereo_distribution'][$dummy['mpeg']['audio']['channelmode']]);
									lib::safe_inc($info['mpeg']['audio']['version_distribution'][$dummy['mpeg']['audio']['version']]);
						...
						$info['mpeg']['audio']['frame_count'] = lib::CastAsInt($framecounter);
	...
	public static function MPEGaudioHeaderDecode($Header4Bytes) {
		...
		$MPEGrawHeader['synch']         = (lib::BigEndian2Int(substr($Header4Bytes, 0, 2)) & 0xFFE0) >> 4;
