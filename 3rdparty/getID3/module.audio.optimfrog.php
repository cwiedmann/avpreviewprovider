...
class optimfrog extends handler
{
	...
	public function Analyze() {
		...
		$this->error('Expecting "*RIFF" or "OFR " at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($OFRheader).'"');
	...
	public function ParseOptimFROGheader42() {
		...
		$OptimFROGencoderVersion_raw   = lib::LittleEndian2Int(substr($OptimFROGheaderData, 0, 1));
		...
		unset($riff, $getid3_temp, $RIFFdata);
	...
	public function ParseOptimFROGheader45() {
		...
		while (!feof($this->getid3->fp) && ($this->ftell() < $info['avdataend'])) {
			...
			$BlockSize   = lib::LittleEndian2Int(substr($BlockData, 4, 4));
			...
			switch ($BlockName) {
				case 'OFR ':
					...
					$thisfile_ofr_thisblock['total_samples']      = lib::LittleEndian2Int(substr($BlockData, $offset, 6));
					$offset += 6;
					$thisfile_ofr_thisblock['raw']['sample_type'] = lib::LittleEndian2Int(substr($BlockData, $offset, 1));
					$thisfile_ofr_thisblock['sample_type']        = $this->OptimFROGsampleTypeLookup($thisfile_ofr_thisblock['raw']['sample_type']);
					$offset += 1;
					$thisfile_ofr_thisblock['channel_config']     = lib::LittleEndian2Int(substr($BlockData, $offset, 1));
					$thisfile_ofr_thisblock['channels']           = $thisfile_ofr_thisblock['channel_config'];
					$offset += 1;
					$thisfile_ofr_thisblock['sample_rate']        = lib::LittleEndian2Int(substr($BlockData, $offset, 4));
					$offset += 4;
					...
					if ($BlockSize > 12) {
						...
						$thisfile_ofr_thisblock['raw']['encoder_id']  = lib::LittleEndian2Int(substr($BlockData, $offset, 2));
						$thisfile_ofr_thisblock['encoder']            = $this->OptimFROGencoderNameLookup($thisfile_ofr_thisblock['raw']['encoder_id']);
						$offset += 2;
						$thisfile_ofr_thisblock['raw']['compression'] = lib::LittleEndian2Int(substr($BlockData, $offset, 1));
						...
						if ((($thisfile_ofr_thisblock['raw']['encoder_id'] & 0xF0) >> 4) == 7) { // v4.507
							if (strtolower(lib::fileextension($info['filename'])) == 'ofs') {
				...
				case 'COMP':
					...
					$COMPdata['crc_32']                       = lib::LittleEndian2Int(substr($BlockData, $offset, 4));
					$offset += 4;
					$COMPdata['sample_count']                 = lib::LittleEndian2Int(substr($BlockData, $offset, 4));
					$offset += 4;
					$COMPdata['raw']['sample_type']           = lib::LittleEndian2Int(substr($BlockData, $offset, 1));
					$COMPdata['sample_type']                  = $this->OptimFROGsampleTypeLookup($COMPdata['raw']['sample_type']);
					$offset += 1;
					$COMPdata['raw']['channel_configuration'] = lib::LittleEndian2Int(substr($BlockData, $offset, 1));
					$COMPdata['channel_configuration']        = $this->OptimFROGchannelConfigurationLookup($COMPdata['raw']['channel_configuration']);
					$offset += 1;
					$COMPdata['raw']['algorithm_id']          = lib::LittleEndian2Int(substr($BlockData, $offset, 2));
					...
					if ($info['ofr']['OFR ']['size'] > 12) {
						...
						// OFR 4.504b or higher
						$COMPdata['raw']['encoder_id']        = lib::LittleEndian2Int(substr($BlockData, $offset, 2));
				...
				case 'MD5 ':
					...
					if ($BlockSize == 16) {
						...
						$thisfile_ofr_thisblock['md5_string'] = lib::PrintHexBytes($thisfile_ofr_thisblock['md5_binary'], true, false, false);
