...
class vqf extends handler
{
	public function Analyze() {
		...
		if ($thisfile_vqf_raw['header_tag'] != $magic) {
			$this->error('Expecting "'.lib::PrintHexBytes($magic).'" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($thisfile_vqf_raw['header_tag']).'"');
			unset($info['vqf']);
		...
		$offset += 8;
		$thisfile_vqf_raw['size']    = lib::BigEndian2Int(substr($VQFheaderData, $offset, 4));
		$offset += 4;
		...
		while ($this->ftell() < $info['avdataend']) {
			...
			$chunkoffset += 4;
			$ChunkSize = lib::BigEndian2Int(substr($ChunkData, $chunkoffset, 4));
			$chunkoffset += 4;
			...
			switch ($ChunkName) {
				case 'COMM':
					...
					$thisfile_vqf_COMM['channel_mode']   = lib::BigEndian2Int(substr($ChunkData, $chunkoffset, 4));
					$chunkoffset += 4;
					$thisfile_vqf_COMM['bitrate']        = lib::BigEndian2Int(substr($ChunkData, $chunkoffset, 4));
					$chunkoffset += 4;
					$thisfile_vqf_COMM['sample_rate']    = lib::BigEndian2Int(substr($ChunkData, $chunkoffset, 4));
					$chunkoffset += 4;
					$thisfile_vqf_COMM['security_level'] = lib::BigEndian2Int(substr($ChunkData, $chunkoffset, 4));
					$chunkoffset += 4;
				...
				case 'DSIZ':
					$thisfile_vqf['DSIZ'] = lib::BigEndian2Int(substr($ChunkData, 8, 4));
					break;
