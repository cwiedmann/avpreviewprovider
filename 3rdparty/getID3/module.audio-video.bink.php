...
$this->error('Bink / Smacker files not properly processed by this version of getID3() ['.$this->getid3->version().']');
		...
		switch ($fileTypeID) {
			...
			default:
				$this->error('Expecting "BIK" or "SMK" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes($fileTypeID).'"');
				return false;
				break;
		}
	...
	public function ParseBink() {
		...
		$info['bink']['data_size']   = lib::LittleEndian2Int(substr($fileData, 4, 4));
		$info['bink']['frame_count'] = lib::LittleEndian2Int(substr($fileData, 8, 2));
