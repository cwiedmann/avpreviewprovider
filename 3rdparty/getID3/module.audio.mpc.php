...
class mpc extends handler
{
	...
	public function Analyze() {
		...
		if (preg_match('#^MPCK#', $info['mpc']['header']['preamble'])) {
			...
			$this->error('Expecting "MP+" or "MPCK" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes(substr($MPCheaderData, 0, 4)).'"');
		...
		while ($offset < $info['avdataend']) {
			...
			switch ($thisPacket['key']) {
				case 'SH': // Stream Header
					...
					$thisPacket['crc']               =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 4));
					$packet_offset += 4;
					$thisPacket['stream_version']    =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 1));
					$packet_offset += 1;
					...
					$otherUsefulData                 =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 2));
					$packet_offset += 2;
				...
				case 'RG': // Replay Gain
					...
					$thisPacket['replaygain_version']     =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 1));
					$packet_offset += 1;
					$thisPacket['replaygain_title_gain']  =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 2));
					$packet_offset += 2;
					$thisPacket['replaygain_title_peak']  =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 2));
					$packet_offset += 2;
					$thisPacket['replaygain_album_gain']  =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 2));
					$packet_offset += 2;
					$thisPacket['replaygain_album_peak']  =       lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 2));
					$packet_offset += 2;
				...
				case 'EI': // Encoder Info
					...
					$profile_pns                 = lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 1));
					$packet_offset += 1;
					...
					$thisPacket['version_major'] = lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 1));
					$packet_offset += 1;
					$thisPacket['version_minor'] = lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 1));
					$packet_offset += 1;
					$thisPacket['version_build'] = lib::BigEndian2Int(substr($MPCheaderData, $packet_offset, 1));
					$packet_offset += 1;
	...
	public function ParseMPCsv7() {
		...
		$StreamVersionByte                           = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 1));
		$offset += 1;
		...
		$thisfile_mpc_header['frame_count']          = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 4));
		$offset += 4;
		...
		$FlagsDWORD1                                   = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 4));
		$offset += 4;
		...
		$thisfile_mpc_header['raw']['title_peak']      = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 2));
		$offset += 2;
		$thisfile_mpc_header['raw']['title_gain']      = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 2), true);
		$offset += 2;

		$thisfile_mpc_header['raw']['album_peak']      = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 2));
		$offset += 2;
		$thisfile_mpc_header['raw']['album_gain']      = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 2), true);
		$offset += 2;

		$FlagsDWORD2                                   = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 4));
		$offset += 4;
		...
		$thisfile_mpc_header['raw']['not_sure_what']   = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 3));
		$offset += 3;
		$thisfile_mpc_header['raw']['encoder_version'] = lib::LittleEndian2Int(substr($MPCheaderData, $offset, 1));
		$offset += 1;
		...
		if ($thisfile_mpc_header['title_peak'] > 0) {
			...
			$info['replay_gain']['track']['peak'] = lib::CastAsInt(round($thisfile_mpc_header['max_level'] * 1.18)); // why? I don't know - see mppdec.c
		}
	...
	public function ParseMPCsv6() {
		...
		$HeaderDWORD[0] = lib::LittleEndian2Int(substr($MPCheaderData, 0, 4));
		$HeaderDWORD[1] = lib::LittleEndian2Int(substr($MPCheaderData, 4, 4));
