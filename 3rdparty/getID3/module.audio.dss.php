...
class dss extends handler
{
	...
	public function Analyze() {
		...
		if (!preg_match('#^[\\x02-\\x06]ds[s2]#', $DSSheader)) {
			$this->error('Expecting "[02-06] 64 73 [73|32]" at offset '.$info['avdataoffset'].', found "'.lib::PrintHexBytes(substr($DSSheader, 0, 4)).'"');
			return false;
		}
		...
		$info['dss']['unknown1']           =   lib::LittleEndian2Int(substr($DSSheader,   28,   4));
		...
		if ($info['dss']['version'] <= 3) {
			$info['dss']['playtime_ms']        =   lib::LittleEndian2Int(substr($DSSheader,  512,   4)); // exact file playtime in milliseconds. Has also been observed at offset 530 in one sample file, with something else (unknown) at offset 512
