...
class bonk extends handler
{
	public function Analyze() {
		...
		if (!lib::intValueSupported($thisfile_bonk['dataend'])) {
			...
			while ($this->BonkIsValidTagName(substr($PossibleBonkTag, 4, 4), true)) {
				$BonkTagSize = lib::LittleEndian2Int(substr($PossibleBonkTag, 0, 4));
				...
				if (($TagHeaderTest{0} != "\x00") || (substr($PossibleBonkTag, 4, 4) != strtolower(substr($PossibleBonkTag, 4, 4)))) {
					$this->error('Expecting "'.lib::PrintHexBytes("\x00".strtoupper(substr($PossibleBonkTag, 4, 4))).'" at offset '.$BonkTagOffset.', found "'.lib::PrintHexBytes($TagHeaderTest).'"');
					return false;
				}
	...
	public function HandleBonkTags($BonkTagName) {
		...
		switch ($BonkTagName) {
			case 'BONK':
				...
				$thisfile_bonk_BONK['version']            =        lib::LittleEndian2Int(substr($BonkData,  5, 1));
				$thisfile_bonk_BONK['number_samples']     =        lib::LittleEndian2Int(substr($BonkData,  6, 4));
				$thisfile_bonk_BONK['sample_rate']        =        lib::LittleEndian2Int(substr($BonkData, 10, 4));

				$thisfile_bonk_BONK['channels']           =        lib::LittleEndian2Int(substr($BonkData, 14, 1));
				$thisfile_bonk_BONK['lossless']           = (bool) lib::LittleEndian2Int(substr($BonkData, 15, 1));
				$thisfile_bonk_BONK['joint_stereo']       = (bool) lib::LittleEndian2Int(substr($BonkData, 16, 1));
				$thisfile_bonk_BONK['number_taps']        =        lib::LittleEndian2Int(substr($BonkData, 17, 2));
				$thisfile_bonk_BONK['downsampling_ratio'] =        lib::LittleEndian2Int(substr($BonkData, 19, 1));
				$thisfile_bonk_BONK['samples_per_packet'] =        lib::LittleEndian2Int(substr($BonkData, 20, 2));
			...
			case 'INFO':
				...
				$thisfile_bonk_INFO['version'] = lib::LittleEndian2Int($this->fread(1));
				...
				if (!$this->BonkIsValidTagName(substr($NextInfoDataPair, 1, 4))) {
					while (!feof($this->getid3->fp)) {
						//$CurrentSeekInfo['offset']  = lib::LittleEndian2Int(substr($NextInfoDataPair, 0, 4));
						//$CurrentSeekInfo['nextbit'] = lib::LittleEndian2Int(substr($NextInfoDataPair, 4, 1));
			...
			case 'META':
				...
				$info['bonk']['META']['version'] = lib::LittleEndian2Int(substr($BonkData,  5, 1));
				...
				for ($i = 0; $i < $MetaTagEntries; $i++) {
					...
					$offset += 4;
					$MetaEntryTagOffset = lib::LittleEndian2Int(substr($BonkData, $offset, 4));
					$offset += 4;
			...
			case ' ID3':
				...
				if (class_exists('id3v2')) {
					$getid3_temp = new getID3();
					$getid3_temp->openfile($this->getid3->filename);
					$id3v2 = new id3v2($getid3_temp);
					$id3v2->StartingOffset = $info['bonk'][' ID3']['offset'] + 2;
					$info['bonk'][' ID3']['valid'] = $id3v2->Analyze();
